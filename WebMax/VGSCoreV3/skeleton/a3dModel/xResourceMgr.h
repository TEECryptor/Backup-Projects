...
protected:
    ...
    virtual bool _loadResource(std::wstring& strResName,ResType*& pRes, int& ResSize , unsigned int arg)
    {
    ...
    virtual bool _unloadResource(std::wstring& strResName, ResType*& pRes, unsigned int& TotalResSize)
    {
        if(pRes == NULL)
            return false;
    ...
    virtual void _deleteResource(std::wstring& strResName, ResType* pRes)
    {
        if(pRes == NULL)
            return ;
        pRes->unload();
...
public:
    ...
    TResHandle(ResType* pResPtr){ m_pResMgr = NULL; m_iData = -1 ; pResPtr = NULL; }
...
public:
    ...
    typedef std::vector<ResSlot >                                                      ResSlots;
...
protected:
    ...
    //排序后的资源优先级改变次数
    typedef std::vector<xResPriority> ResPrioritys;

...
BOOL CGridCtrl::PasteTextToGrid(CCellID cell, COleDataObject* pDataObject)
{
	...
	TCHAR tcBuffer[512] = {0};
	int iLen = MultiByteToWideChar(CP_ACP, 0, szBuffer, sizeof(szBuffer), tcBuffer, 512);
	if (iLen < 512) tcBuffer[iLen + 1] = _T('\0');
    CString strText = tcBuffer;
    delete szBuffer;
...
//欢迎使用和改进。
{
	xNum=fabs((float)xNum);
	yNum=fabs((float)yNum);

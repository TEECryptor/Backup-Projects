...
void CWriteBuffer::seekw(int _offset, xcd_seek_dir dir)
{
	if(dir == xcdsd_cur)
	{
...
void CWriteBuffer::seekg(int _offset, xcd_seek_dir dir)
{
	if(dir == xcdsd_cur)
	{
...
int    CReadBuffer::read(_xcd_int8* buf, int byte_read)
{
	...
	m_DiskFile->seek(m_FileNowPos,xcdsd_beg);
...
void   CReadBuffer::seekg(int _offset, xcd_seek_dir pos)
{
	if(pos == xcdsd_cur)
	{

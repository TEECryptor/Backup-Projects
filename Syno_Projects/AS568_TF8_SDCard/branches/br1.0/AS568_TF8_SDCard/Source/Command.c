...
UINT8 SMS4MAC(UINT8 *key, UINT8 *IV)
{
	...
	UINT8 tempIV[16] = {0};
	UINT8 OldMac[4] = {0};
    UINT8 SynoCmdFlags[4] = {0xEF, 0x01, 0xE1, 0x00};
    ...
    // 组装整个APDU数据包
    newlen = 0;
    memcpy(MacData, SynoCmdFlags, 4);
    newlen += 4;
    //
    nApduLen = sizeof(APDUHEADER_S) + gApduBody.LC + 3;
    MacData[newlen] = nApduLen & 0x00FF;    
    MacData[newlen + 1] = (nApduLen >> 8) & 0x00FF;  
    newlen += 2;
    //
	memcpy(MacData + newlen, &gApduHeader, sizeof(APDUHEADER_S));
    newlen += sizeof(APDUHEADER_S);
    //
    MacData[newlen] = 0x00;         //需要做MAC的命令，Lc长度均为3且没有Le!!!
	MacData[newlen + 1] = (gApduBody.LC >> 8) & 0x00FF;				
	MacData[newlen + 2] = gApduBody.LC & 0x00FF;																	
    newlen += 3;
    //  
	memcpy(MacData + newlen, gApduBody.DATA, gApduBody.LC - 4);
    newlen += (gApduBody.LC - 4); 
    // 
	MacData[newlen] = 0x80;
    newlen += 1;
    //DBG_INFO("APDU data:");
...
{                   
    ...
    //生成ECC密钥对
	keylen = (keybitlen/8);
	pub.X = gApduBody.DATA;
	pub.Y = gApduBody.DATA + keylen;
	prv.PrivateKey = gApduBody.DATA + keylen*2; 		

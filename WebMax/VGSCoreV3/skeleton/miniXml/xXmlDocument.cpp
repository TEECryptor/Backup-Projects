...
#if _MSC_VER >= 1310 
	swprintf(buf,32,L"%d",newValue);
#else
	swprintf(buf,L"%d",newValue);
#endif
...
void   xXmlAttribute::setHex(unsigned int newValue)
{
	wchar_t buf[32];
#if _MSC_VER >= 1310 
	swprintf(buf,32,L"0x%x",newValue);
#else
	swprintf(buf, L"0x%x",newValue);
#endif
...
#if _MSC_VER >= 1310 
	swprintf(buf,32,L"%f",newValue);
#else
	swprintf(buf,   L"%f",newValue);
#endif
...
void xXmlNode::cloneTo(xXmlNode* pDestNode, bool cloneChild)
{
	...
	for(size_t iCData = 0 ; iCData < nCDatas ;  iCData ++)
	{
		xXmlCData* pCData = m_CDatas[iCData];
	...
	if(cloneChild)
	{
		...
		for(size_t iChild = 0 ; iChild < nChildrens ; iChild ++)
		{
			xXmlNode* pChildNode = m_ChildrenNodes[iChild];
...
void xXmlNode::clear()
{
	...
	for(int iCData = 0; iCData <  (int)m_CDatas.size() ; ++iCData)
	{
		xXmlCData* pCData = m_CDatas[iCData];
		delete pCData;
	}
...
bool xXmlDocument::load(const unsigned char*    buf , int len , bool loadHeader)
{
...
bool xXmlDocument::load(const unsigned char*    buf , size_t len , bool loadHeader)
{
...
{
	ofstream file;
    char _ansiName[1024] = {0};
	XEvol_UnicodeToLocale(fileName , _ansiName , 1024);
	file.open(_ansiName,ios::binary);
	if(file.fail())
		return false;
...
static wstring    makeEscapeString(wstring _in)
{
	wstring ret;
...
template <typename _TOutStream > void saveToStream(bool bSaveAsXML , _TOutStream& stream , xXmlNode* pNode,int level)
{
	...
	if(bSaveAsXML == false && pNode->countCData() == 0)
	{
		...
		{
			saveToStream(bSaveAsXML, stream,nodes[iNode] , level + 1);
		}

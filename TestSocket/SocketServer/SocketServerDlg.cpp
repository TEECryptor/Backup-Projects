...
BOOL CSocketServerDlg::OnInitDialog()
{
	...
	SetDlgItemText(IDC_STATIC_STATE, _T("客户端连接状态:未连接!"));
...
{
	SetDlgItemText(IDC_STATIC_STATE, _T("客户端连接状态:已连接!"));
		...
		case MSG_TEST:		//Client information data message
			{
				...
				char szParam1[32];
				sprintf(szParam1, "参数1:%d", lpSocketMsg->iParam1);
				SetDlgItemText(IDC_STATIC_PARAM1, szParam1);
				//
				char szParam2[32];
				sprintf(szParam2, "参数2:%f", lpSocketMsg->fParam2);
				SetDlgItemText(IDC_STATIC_PARAM2, szParam2);
				//
				char szParam3[32];
				sprintf(szParam3, "参数3:%s", lpSocketMsg->szParam3);

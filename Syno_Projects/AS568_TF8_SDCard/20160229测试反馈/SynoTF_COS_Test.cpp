...
//	SKF_ECCExportSessionKey(hCon, SGD_SM1_ECB, &stEccSignPubKeyBlob, &stEccCipherBlob, &hSessionKey);
	//	使用签名公钥对会话密钥加密
...
void test_EccEncryptDecrypt(DEVHANDLE hDev)
{
	...
	HCONTAINER hCon = NULL;
	HANDLE hSessionKey = 0;
	ECCPUBLICKEYBLOB stEccPubKeyBlob = {0}, stSignPubKey;
	...
	//生成签名公私钥
	ulRes = SKF_GenECCKeyPair(hCon, SGD_SM2_1, &stSignPubKey);
	if (0 != ulRes)
	{
		printf("SKF_GenECCKeyPair() failed! ulRes = 0x%x\n", ulRes);
		goto END;
...
{	
	...
	//	goto END;
	//}
	ulRes = SKF_GenECCKeyPair(hCon, SGD_SM2_1, &stEccPubKeyBlob);
	...
	//ulHashLen = 32; 
	//memset(btHashData, 0x11, ulHashLen);
...
void test_SymmKeyData(DEVHANDLE hDev)
{
	...
	//}
	memset(btSessionKey, 0x11, 32);
	memset(g_btPlainData, 0x55, TOTAL_DATA_LEN);

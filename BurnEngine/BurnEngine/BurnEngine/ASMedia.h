...
public:
	...
	//-------------------------------------------
public:
	//Get the media current state (File_IsMounted, File_IsDirty, etc)
	DWORD		GetMediaState();										//Waiting for deleting!!!!!
	//Get current media block size, in 4 byte
	DWORD		GetBlockSize();
	//Get current media free block, in 4 byte
	DWORD		GetFreeBlocks();
	//-------------------------------------------
	//Open this volume

...
software_noisemaker::software_noisemaker(int sX, int sY, parameterhandler *prm, const LPDIRECT3DDEVICE9 dev, BOOL bSupportGPUNormal)
{
	depthstencil =  NULL;
	packed_noise_texture[0] = NULL;
    packed_noise_texture[1] = NULL;
	heightmap = NULL;
	normalmap = NULL;

	//initial effect object
	hmap_effect = NULL;
	nmap_effect = NULL;
...
float software_noisemaker::get_height_at(float u, float v)
{
	...
	return get_height_dual((int)(magnitude*u), (int)(magnitude*v));
}
...
void software_noisemaker::generate_normalmap( )
{
	if(m_bSupportGPUNormal)
	{
		...
		SAFE_RELEASE_D3D(target);
		SAFE_RELEASE_D3D(bb);
		SAFE_RELEASE_D3D(old_depthstencil);
	}
}
...
void software_noisemaker::load_effects()
{
	...
	FindMediaFile(realFilePath, "v2_heightmapgen.fx");
	D3DXCreateEffectFromFile(device, realFilePath, 
		NULL, NULL, 0, NULL, &hmap_effect, &errors );*/


	//从特效缓存中创建特效
	HRESULT hr1 =   D3DXCreateEffect(device, g_FXHeightMapGenBuffer, 
		(sizeof(g_FXHeightMapGenBuffer) - 1), NULL, NULL, 
		NULL, NULL, &hmap_effect, &errors);

	if (errors != NULL){
		errortext = (char*) errors->GetBufferPointer();
		MessageBox(NULL, (errortext), TEXT("hmap_effect"), MB_OK);		
	}

	if (SUCCEEDED(hmap_effect->FindNextValidTechnique(NULL, &hTechnique)))    
		hmap_effect->SetTechnique(hTechnique);

	// load effect
	/*FindMediaFile(realFilePath, "v2_normalmapgen.fx");
	D3DXCreateEffectFromFile(device, realFilePath, 
		NULL, NULL, 0, NULL, &nmap_effect, &errors );*/
    	//从特效缓存中创建特效
	hr1 =   D3DXCreateEffect(device, g_FXNormalMapGenBuffer, 
		(sizeof(g_FXNormalMapGenBuffer) - 1), NULL, NULL, 
		NULL, NULL, &nmap_effect, &errors);

	if (errors != NULL){
		errortext = (char*) errors->GetBufferPointer();
		MessageBox(NULL, (errortext), TEXT("nmap_effect"), MB_OK);		
	}
...
void software_noisemaker::Invalidate()
{
	...
	//释放贴图对象
	SAFE_RELEASE_D3D(packed_noise_texture[0]);
	SAFE_RELEASE_D3D(packed_noise_texture[1]);

	SAFE_RELEASE_D3D(heightmap);
	SAFE_RELEASE_D3D(normalmap);

	//释放效果对象
	SAFE_RELEASE_D3D(hmap_effect);
	SAFE_RELEASE_D3D(nmap_effect);
}

void software_noisemaker::RestoreDeviceObject()
{
	init_textures();
	//创建效果对象
	load_effects();
...
//释放
void software_noisemaker::Release()
{	
	Invalidate();
}

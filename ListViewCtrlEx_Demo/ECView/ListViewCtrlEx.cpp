 ...
 * ListViewCtrlEx.cpp
 * 
 * Implementation of class "CListBase"
 ...
 * Thomas Holte
 * 
 * Copyright (c) 2009, 2010 Thomas Holte
 * All Rights Reserved
 * 
 */
...
/*** A notification message has been received ********************************/
BOOL CListCtrlEx::OnChildNotify(
  UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
  CListBase::OnChildNotify(message, lParam);
...
/*** A notification message has been received ********************************/
BOOL CListViewEx::OnChildNotify(
  UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
  CListBase::OnChildNotify(message, lParam);
...
{
  public:
  LRESULT Default() {return __super::Default();}
    ...
    if (pimglst)
    {
      ...
      if (pimglst->GetImageInfo(pItem->iImage, &imgInfo))
      {
        IMAGELISTDRAWPARAMS imldp =
        {
          ...
          /* fState   = */ ILS_NORMAL,
          /* Frame    = */ 0,
          /* crEffect = */ CLR_DEFAULT
        };

        pimglst->DrawIndirect(&imldp);

        // Doesn't work with MFC 10 due to an error in the MFC method
...
void CListBase::EnableSortIcon(BOOL bEnable, int nSortColumn)
{
  ...
  if (nSortColumn != 0) m_iSortColumn = nSortColumn;
  if (bEnable && m_visualStyle != Present)
  {
    CreateSortIcons                           ();
...
void CListBase::SetSortColumn(int nColumn)
{
  ...
  if (nColumn != m_iSortColumn)
  {
    ...
    SetSortIcon();
    if (bColumnChanged)
      if (m_visualStyle == Present && m_bColorSortColumn)
        if (m_iSortColumn != 0)
          m_pListCtrl->SendMessage(
            LVM_SETSELECTEDCOLUMN, GetPhysicalIndex(abs(m_iSortColumn)-1));
        else
...
/*** A notification message has been received ********************************/
void CListBase::OnChildNotify(UINT message, LPARAM lParam)
{
  if (message == WM_NOTIFY)
  {
    ...
    if (pNm->hdr.code == LVN_COLUMNCLICK)
      pNm->iSubItem = GetLogicalIndex(pNm->iSubItem);
  }
}
...
BOOL CListBase::OnColumnclick(NMHDR* pNMHDR, LRESULT* pResult) 
{
  ...
  SetSortIcon();
  if (m_bColorSortColumn && bSortColumnChanged && m_visualStyle == Present)
...
void CListBase::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult) 
{
  ...
  switch (pNMLVCustomDraw->nmcd.dwDrawStage)
  {
    ...
    case CDDS_ITEMPOSTPAINT | CDDS_SUBITEM:
    {
      ...
      // consider header control
      if (rc.top < rcItem.bottom)
      {
        rc.top = rcItem.bottom;
        if (rc.top > rc.bottom) break;  // item covered by header control?
      }
...
BOOL CListBase::OnEraseBkgnd(CDC* pDC) 
{
  ...
  else if (m_aColumnData.GetSize() == 0 && m_pListCtrl->GetItemCount() <= 0)
  {
    // special treatment for empty list view controls due to a display bug
    // under Windows 7
    CRect rc;
    pDC->GetClipBox(rc);
    pDC->PatBlt    (rc.left, rc.top, rc.Width(), rc.Height(), PATCOPY);
...
LRESULT CListBase::OnGetItem(LPARAM lParam)
{
  ...
  if (lResult                                    &&
      !(m_pListCtrl->GetStyle() & LVS_OWNERDATA) &&
      (!bVisible || pLVItem->mask != LVIF_STATE))
  {
      ...
      switch (pNMHdr->code)
      {
        ...
        case HDN_DIVIDERDBLCLICKA:
              ...
              ))
          {
            ...
            m_pListCtrl->DefWindowProc(
              LVM_SETCOLUMNWIDTH, pNMHeader->iItem, iOptWidth);
            bRet = TRUE;
          }
          break;
        ...
        case HDN_ITEMCLICK:
          m_pListCtrl->Default();
          // Suppress further processing of this message to prevent Windows
          // from sending the notification message LVN_COLUMNCLICK a second
          // time! For unknown reasons this is necessary in an Unicode
          // environment only.
          bRet = TRUE;
          break;

...
LONG CHttpDownloadTask::OpenNetFile()
{
	...
	try
	{
		...
		m_hNetFile = HttpOpenRequest(m_hConnect,
									 ...
									 NULL,
									 acceptTYPE,
									 INTERNET_FLAG_DONT_CACHE|INTERNET_FLAG_RELOAD|INTERNET_FLAG_NO_CACHE_WRITE,
									 0);
		if (NULL != m_hNetFile)
		{
...
BOOL CHttpDownloadTask::CanUseMultiThread()
{
	...
	if (!m_stuDownloadInfo.bForceUseSingleThread && 
...
LONG CHttpDownloadTask::OpenTargetFile(DWORD dwFileSize, BOOL bOverWrite)
{
	...
	try
	{
		...
		// If server can't use multi-thread, we need to download 
		// it from file begin by single thread
...
LONG CHttpDownloadTask::CreateDownloadThreads()
{
	...
	if (!CanUseMultiThread())
	{
		...
		m_pDownloadThreads[0].SetDownloadInfo(m_hNetFile, m_hTargetFile, m_pLogFile, &info);
...
LONG CHttpDownloadTask::OnProgress()
{
	...
	// If speed has been 0 for 5 seconds, try to auto-restart 
	if (m_dwZeroBytesTime >= 5)
	{
		PostThreadMessage(m_dwProcessThreadID, PROC_MSG_RESTART, -1, 0);
		HTTPLOG(m_pLogFile, "ERROR --> OnProgress(): Have not download any data for 5 seconds, try to retart task!"); 
	}

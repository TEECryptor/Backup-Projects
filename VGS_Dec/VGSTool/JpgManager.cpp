...
//            AfxThrowUserException();
        ...
        // Setup JPEG
		int iBuffLen = m_iImageWidth*m_iImageHeight*3;
		BYTE* pTmp = new BYTE[iBuffLen];
        image.JPGFile          = const_cast<char*>("no name");
        image.JPGWidth         = m_iImageWidth;
        image.JPGHeight        = m_iImageHeight;
		image.JPGBytes		   = pTmp;
		image.JPGSizeBytes	   = iBuffLen;
		...
		int iRet = 0;
        if( (iRet = ijlWrite( &image, IJL_JBUFF_WRITEWHOLEIMAGE )) != IJL_OK )
        {
			iJpgSize = image.JPGSizeBytes;

			char szTmp[MAX_PATH] = {0};
			sprintf(szTmp, "Save to jpg buffer error!\nError code:%d", iRet);
			MessageBox(NULL, szTmp, "Error", MB_OK);
//            TRACE( "Can't write image\n" );
//            AfxThrowUserException();
        }

		//Save the image data and size
		m_iImageSize = image.JPGSizeBytes;
		if(m_pImageData) delete []m_pImageData;
		m_pImageData = new BYTE[m_iImageSize];
		memcpy(m_pImageData, pTmp, m_iImageSize);

...
bool  xComDocument::open(IStreamIO* pReader,xcd_rwmode /*mode*/,int /*_offset*/, bool load_to_mem)
{
	if(pReader == NULL)
		return false;
...
xcomdocstream* xComDocument::open_stream(xcd_data_type type , int type_index,bool /*not_create*/)
{
	assert(failed() == false);
	int count = 0 ;
	size_t _nEntrys = m_DataEntrys.size();
	for(size_t i = 0 ; i < _nEntrys; ++i)
	{
		DataEntry& entry = m_DataEntrys[  i ];
...
int        xComDocument::count_stream(xcd_data_type type)
{
	assert(failed() == false);
	int count = 0 ;
	size_t nEntrys = m_DataEntrys.size();
...
const wchar_t*   xComDocument::name()
{
	if(wcslen(m_file_name) == 0)
		return L"No Name";
	return m_file_name;
}
...
bool xComDocument::add_file(const wchar_t* bas_dir,const wchar_t* file_name,int compressed_rate,xcd_data_type type)
{
	...
	{
		item_name = std::wstring(bas_dir) + file_name_noext;
	}
	else
	{
		item_name = std::wstring(bas_dir) + L"/"+ file_name_noext;
	}

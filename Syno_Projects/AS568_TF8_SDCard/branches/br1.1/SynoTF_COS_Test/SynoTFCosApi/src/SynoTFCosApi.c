...
ULONG _AddPKCS7Padding(BYTE* lpPadded, ULONG ulPaddedLen)
{
	BYTE i = 0;
	BYTE btPad = 0;

	if (!lpPadded || ulPaddedLen > 16)
	{
		return SAR_INVALIDPARAMERR;
	}

	btPad = (BYTE)ulPaddedLen;
	for (i = 0; i < btPad; i++)
	{
		lpPadded[i] = btPad;
	}
...
ULONG _RemovePKCS7Padding(BYTE* lpData, ULONG *pulDataLen)
{
	BYTE n = 0;
	BYTE btPad = 0;
	BOOL bPadded = TRUE;
    ...
    BYTE *pbData, ULONG ulDataLen, BYTE *pbEncryptedData, ULONG *pulEncryptedLen)
{
	...
	if (g_ulPaddingType)
	{
		...
		_AddPKCS7Padding(aucPad, ulPadByte);
	}
	else
	{
		if (ulDataLen % 16)
    ...
    BYTE *pbData, ULONG ulDataLen, BYTE *pbEncryptedData, ULONG *pulEncryptedLen)
{
	...
	if (g_ulPaddingType)
	{
		if (ulDataLen % 16)
		{
			...
			_AddPKCS7Padding(g_aucPadByte, g_ulPadLen);
		}
		else
		{
			g_ulPadLen = 16;
    ...
    BYTE *pbEncryptedData, ULONG ulEncryptedLen, BYTE *pbData, ULONG *pulDataLen)
{
	...
	*pulDataLen = ulEncryptedLen;
	...
	if (g_ulPaddingType)
	{
		_RemovePKCS7Padding(pbData, pulDataLen);
		g_ulPaddingType = 0;
	}
    ...
    BYTE *pbDecryptedData, ULONG *pulDecryptedDataLen)
{
	...
	if (g_ulPaddingType)
	{
		_RemovePKCS7Padding(pbDecryptedData, pulDecryptedDataLen);
		g_ulPaddingType = 0;
	}
    ...
    BYTE *pbData, ULONG ulDataLen, BYTE *pbEncryptedData, ULONG *pulEncryptedLen)
{
	...
	if (g_ulPadLen > 0 && g_ulPadLen != 0x10)
	{
		lpPaddedData =  (BYTE*)ICA_malloc(ulDataLen + g_ulPadLen);
    ...
    BYTE *pbData, ULONG ulDataLen, BYTE *pbDecryptedData, ULONG *pulDecryptedLen)
{
	...
	if (g_ulPaddingType)
	{
		_RemovePKCS7Padding(pbDecryptedData, pulDecryptedLen);
	}

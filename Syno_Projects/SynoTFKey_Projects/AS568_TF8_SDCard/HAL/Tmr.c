...
UINT8 TMR_Init(UINT8 nTimerNum, UINT32 nLimit, UINT8 nClkSrc, UINT8 nMode)
{
	...
	{
		if(nTimerNum == T0)
			GpioRegs.GPAUSE3.bit.GPA24 = MUX_TMR;
		else if(nTimerNum == T1)
			GpioRegs.GPAUSE3.bit.GPA25 = MUX_TMR;
		else
			GpioRegs.GPAUSE3.bit.GPA26 = MUX_TMR;
	} 
...
UINT8 TMR_Stop(UINT8 nTimerNum)
{
	...
	pTimer->CONTROL.all = (CONTROL_WKEY|(pTimer->CONTROL.all) )& ~(1<<CONTROL_EN);	
...
{	
	...
	pTimer->CONTROL.all = (CONTROL_WKEY | CONTROL_WUFLAG | RegControlVal)& ~(1<<CONTROL_EN);
	pTimer->COUNT = 0x00000000;

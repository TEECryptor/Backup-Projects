...
public:
    ...
    virtual size_t seek(long offs, xcd_seek_dir _dir) = 0;
    virtual size_t tell() = 0;
    virtual int    close() = 0;
    virtual bool   eof() = 0;
public:
    template <typename T> size_t t_write(const T& data)
    {
        return write( (const char*) &data,sizeof(T));
    }

    template <typename T> size_t t_write(const T* data, size_t nObject)
    {
        return write( (const char*)data,sizeof(T)*nObject);
    }

    template <typename T> size_t t_read(T& data)
    {
        return read( (char*) &data,sizeof(T));
    }

    template <typename T> size_t t_read(T* data, size_t nObject)
    {
...
public:
    ...
    virtual void        seekg(int _offset, xcd_seek_dir dir) = 0;

    virtual void        seekw(int _offset, xcd_seek_dir dir) = 0;
    virtual int         tellg() = 0;
    virtual int         tellw() = 0;

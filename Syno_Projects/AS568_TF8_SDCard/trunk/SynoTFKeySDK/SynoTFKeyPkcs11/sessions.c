...
static char RCSID[]="$Id: sessions.c,v 1.11 2000/09/19 09:14:54 lbe Exp $";
...
)
{
    ...
    * No sense opening a session in the void does now? B-) */
    ...
    //rv = TOK_OpenSession(new_session);
	rv = slot_data->methods->OpenSession(new_session);
    if(rv != CKR_OK)
    {
      ...
      )
{
    ...
    */
    ...
    {
        session_data->slot_data->flags &= ~(CK_I_SIF_USER_SESSION|CK_I_SIF_RW_SO_SESSION);
    }
...
CK_RV CI_PropagateSessionState(
...
)
{
	...
	/* call token method to login */
	CK_I_CALL_TOKEN_METHOD(rv, Login, (session_data, userType, pPin, ulPinLen));
    if(rv != CKR_OK)
    {
        CI_LogEntry("C_Login", "Call Login() failed!", rv, 0);
        return rv;
    }
...
)
{
    ...
    if(rv != CKR_OK)
    {
        CI_LogEntry("C_Login", "Call CI_ReturnSession() failed!", rv, 0);
        return rv;
    }
	
	/* call token method to logout */
	CK_I_CALL_TOKEN_METHOD(rv, Logout, (session_data));
    if(rv != CKR_OK)
    {
        CI_LogEntry("C_Login", "Call Logout() failed!", rv, 0);
        return rv;
    }

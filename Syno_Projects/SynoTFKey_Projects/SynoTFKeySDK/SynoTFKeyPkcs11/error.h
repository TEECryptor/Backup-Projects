...
#else
#define CI_LogEntry(f,p,r,l) ICA_LogI("%s %s",p, CI_ErrorStr(r))//do{}while(0)
#define CI_SetLogingLevel(l) do{}while(0)
#define CI_VarLogEntry(f,p,r,l,...) ICA_LogI("%s %s",p, CI_ErrorStr(r))//do{}while(0)
#define CI_CodeFktEntry(f,p,...) ICA_LogI("%s",p)//do{}while(0)
...
#ifndef WIN32
#define TC_free(handle) ICA_FREE(handle)//free(handle)
#define TC_calloc(nelem, elsize) ICA_calloc(nelem, elsize)//calloc(nelem,elsize)
#define TC_malloc(size) ICA_malloc(size)//malloc(size)
#else
#define TC_free(handle) ICA_free(handle)

    ...
    UCHAR *pInBuf, ULONG ulInLen, UCHAR *pOutBuf, ULONG *pulOutLen)
{
    ...
    if (stCmdHeader.CLA & 0x04)
    {
		...
		/* 生成8Bytes随机数 */
		ulRecvLen = 8;
		ulRet = CMD_Transmit(hHandle, CMD_GEN_RANDOM, 0x00, 0x00, NULL, 0, acRandom, &ulRecvLen);
        if (SAR_OK != ulRet)
    	{
    		return ulRet;
    	}
		...
		/* APDU的数据长度Lc */
		memcpy(&usApduLc, pInBuf + CMD_HEAD_LEN, 1);	//CLA=0x84的3个命令,Lc长度均为1且Le为0
		...
		tSM4Buf[tLen + 1] = 0x00;
		tLen += 2;
		memcpy(tSM4Buf + tLen, pInBuf + CMD_HEAD_LEN + 1, ulInLen - CMD_HEAD_LEN - 1);
		tLen += ((USHORT)ulInLen - CMD_HEAD_LEN - 1);
		//
		usApduLc += 4;
		acBuf[ulBufLen] = usApduLc;
		ulBufLen += 1;

		/* APDU的数据和Le(实际上做MAC运算的命令都没有Le且Lc的长度为1) */
		memcpy(acBuf + ulBufLen, pInBuf + (CMD_HEAD_LEN + 1), (ulInLen - CMD_HEAD_LEN - 1));
		ulBufLen += (ulInLen - CMD_HEAD_LEN - 1);
	...
	{
        SYNO_LogD("pulOutLen=%d, usRespApduLen=%d", *pulOutLen, usRespApduLen);
	...
	*pulOutLen = usRespApduLen;
    ...
    SYNO_LogD("APDUInterface ucSW = <%x>", ucSW);
    ...
    UCHAR *pInBuf, ULONG ulInLen, UCHAR *pOutBuf, ULONG *pulOutLen, CMDTYPE_E eCmdType)
{
	...
	/* 计算实际发送的数据长度，必须为512的整数倍 */
	ulSendLen = (ulInLen % 512) ? ulInLen + (512 - ulInLen % 512) : ulInLen;
	if (*pulOutLen < ulSendLen)
	{
        LOG_ErrNo(SAR_INVALIDPARAMERR);
        return SAR_INVALIDPARAMERR;
	}
    ...
    /* 发送数据 */
    ulRet = SendPackage(hHandle, pInBuf, ulSendLen, eCmdType);
	if (SAR_OK != ulRet)
	{
		return ulRet;
	}
    ...
    UCHAR *pucIn, ULONG ulInLen, UCHAR *pucOut, ULONG *pulOutLen)
{
	...
	if (usLc > 0)
	{
		if ((usLc < 256) && (usLe <= 256))					//change by comon @ 20151224
		{
			acApduBuf[ulApduLen] = (UCHAR)usLc;
			ulApduLen += 1;			
		}
		else if ((usLc < 256) && (usLe > 256))
		{
	...
	if (usLe > 0)
	{
		...
		else
		{
			if (usLe < 256)
			{
				acApduBuf[ulApduLen] = (UCHAR)usLe;
				ulApduLen += 1;
			}
			else if (usLe == 256)
			{
			...
			else
			{
				...
				acApduBuf[ulApduLen + 2] = usLe & 0x00ff;
				ulApduLen += 3;
			}
		}
	}

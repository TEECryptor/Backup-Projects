...
int FormateB64String(LPCWSTR pwzB64, DWORD dwLen, MemoryBlock<WCHAR>& mbB64Formated)
{
	...
	if (wcschr(pwzB64, L'\r') == 0 && wcschr(pwzB64, L'\n') == 0)
		return 0;
	int nB64Index = 0;
	for (UINT i = 0; i < dwLen; i++)
	{
		if ((pwzB64[i] >= L'A' && pwzB64[i] <=L'Z') 
			|| (pwzB64[i] >=L'a' && pwzB64[i] <= L'z') 
			|| (pwzB64[i] >= L'0' && pwzB64[i] <= L'9') 
			|| pwzB64[i] == L'+' || pwzB64[i] == L'/' || pwzB64[i] == L'=')
		{
			*(mbB64Formated.GetPtr() + nB64Index) = pwzB64[i];
			nB64Index++;
		}
		else if (pwzB64[i] == L'\r' || pwzB64[i] == L'\n')
			continue;
		else
		{
			return 0;

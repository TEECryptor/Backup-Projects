...
)
{
    ...
    {
        rv = ICA_TOK_GenerateKey(session_data, pMechanism, new_key);
        if(rv != CKR_OK)
        {
            CI_ObjDestroyObj(new_key);
            return rv;
...
)
{
    ...
    {
        rv = ICA_TOK_GenerateKeyPair(session_data, pMechanism,
            new_public_key, new_private_key);
        if(rv != CKR_OK)
        {
            CI_ObjDestroyObj(new_private_key);
...
)
{
  ...
  CK_I_SESSION_DATA_PTR session_data = NULL_PTR;
    ...
    {
        rv = ICA_TOK_WrapKey(session_data, pMechanism, wrapper,
            key_obj, pWrappedKey, pulWrappedKeyLen);
    }
...
)
{
   ...
   * they will be modified */
    ...
    {
        rv = ICA_TOK_UnwrapKey(session_data, pMechanism, wrapper,
        	 pWrappedKey, ulWrappedKeyLen, key_obj);
        if(rv != CKR_OK)
        {
            CI_ObjDestroyObj(key_obj);

...
BEGIN_MESSAGE_MAP(CASStorageDemoDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BTN_EJECT, OnBnClickedBtnEject)
END_MESSAGE_MAP()
//--------------------------------------------------
...
//--------------------------------------------------
void CASStorageDemoDlg::OnBnClickedBtnEject()
{
	if(NULL == m_pBurnEngine || NULL == m_pDriveManager)
		return;
	...
	for(int iItem = 0; iItem < pDrivesLstCtrl->GetItemCount(); iItem++)
	{
		if(TRUE == pDrivesLstCtrl->GetCheck(iItem))
		{
			int iSelDriveID = (int)pDrivesLstCtrl->GetItemData(iItem);
			CDrive* pSelDrive = m_pDriveManager->GetDriveByID(iSelDriveID);
			if(NULL != pSelDrive)
			{
				if(TRUE == bEject)
				{

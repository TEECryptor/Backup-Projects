...
void ExtractToDirectory(IComDocBase* _doc,const wchar_t* bas_dir)
{
	...
	for(int iEntry = 0 ; iEntry < nEntrys; ++iEntry)
	{
		xComDocument::DataEntry& entry = pDoc->m_DataEntrys[iEntry];
		...
		if(! xcd_is_dir(entry.m_DataEntryItem.m_DataType))
		{
			...
			char _ansiFileName[1024] = {0};
			XEvol_UnicodeToLocale(file_name.c_str() , _ansiFileName , 1024);
	        ofstream out(_ansiFileName,ios::binary);

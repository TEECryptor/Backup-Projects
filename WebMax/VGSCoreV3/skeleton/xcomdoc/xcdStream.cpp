...
void  XComDocStreamImp::_read_all_data_from_dataprovider_src_file()
{
	if(m_bUseSrcFile)
	{
		...
		if(fStream != NULL)
		{
			...
			m_pWriteBuffer->seekw(len,xcdsd_beg);
...
void  XComDocStreamImp::_relase_all_data_from_dataprovider_src_file()
{
	if(m_bUseSrcFile)
	{
		m_pWriteBuffer->seekw(0,xcdsd_beg);
		m_pWriteBuffer->seekg(0,xcdsd_beg);
		m_pWriteBuffer->free();
	}
}
...
size_t XComDocStreamImp::_read_all_data_from_file()
{
	if(_is_compressed())
	{
		...
		m_compressedReadBuffer.seekg(0,xcdsd_beg);
	...
	m_ReadBuffer.seekg(0,xcdsd_beg);
...
size_t XComDocStreamImp::data_len()
{
	if(m_bUseSrcFile)
	{
		...
		XEvol_UnicodeToLocale(m_SrcFileName.c_str() , fileNameAnsi , 256);
...
void XComDocStreamImp::stream_seekr(xcd_seek_dir dir,int _offset)
{
	if(m_DataInMemBuffer == false)
	{
		if(_is_compressed())
			m_compressedReadBuffer.seekg(_offset,dir);
		else
			m_ReadBuffer.seekg(_offset,dir);
	}
	else
	{
		m_pWriteBuffer->seekg(_offset,dir);
	}
}

void XComDocStreamImp::stream_seekw(xcd_seek_dir dir ,int _offset)
{
	if(m_DataInMemBuffer == false)
	{
		return ;
	...
	{
		m_pWriteBuffer->seekw(_offset,dir);
		return ;
	}
}

...
* Date         :	2012/07/26
* Version      :    v1.0
* Description  :    I2c Hardware Abstract Layer compatible with SY03
* ModifyRecord :
****************************************************************************/
/***************************************************************************
 ...
 * Input:		UINT8 bI2cNum -- 	I2C number, I2C0 for SY03;
 * 				UINT8 bDevAddr --	I2C device address (7 bit address mode);
 * 				UINT8 bIndex -- 	Index of I2C device;
 * 				UINT8 *pData -- 	the data read back;
		...
		if(pI2c->CSTAT.bit.Si2cBusy && pI2c->CSTAT.bit.Si2cRw && (!pI2c->CSTAT.bit.Si2cNack))
		{
			...
			while(!pI2c->CSTAT.bit.TxFull)	// till FIFO is full
			{
				...
				else
				{
					...
					while(!pI2c->CSTAT.bit.TxEpt && (times--)); 	// wait transmit complete
					pI2c->GCTL.bit.RxEn = 1;
					pI2c->GCTL.bit.TxEn = 0;
					break;
				}

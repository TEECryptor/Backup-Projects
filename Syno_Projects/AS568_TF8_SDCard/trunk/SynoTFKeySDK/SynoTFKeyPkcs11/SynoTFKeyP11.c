/***************************************************************************
 *	File name:SynoTFKeyP11.c
 *	Introduce:The implement file for PKCS11 type functions
 *	Author:Syno common
 *	Date:2015/11/18
 *  Version:v1.0
 **************************************************************************/
...
static BOOL g_bWaitFlag = TRUE;
static ULONG g_ulRemovedNameLen = 0;
static ULONG g_ulInsertedNameLen = 0;
static CHAR g_csRemovedNameList[512] = {0};
static CHAR g_csInsertedNameList[512] = {0};

/*----------------------------------------------*
 * 设备管理                                     *
 *----------------------------------------------*/

ULONG P11_WaitForDevEvent(LPSTR szDevName,
    ULONG *pulDevNameLen, ULONG *pulEvent)
{
	ULONG ulRes = SAR_OK;
	ULONG ulLastNameListLen = 1024;
	ULONG ulNewNameListLen = 1024;
	BOOL bExistDevice = FALSE;
	BOOL bIsRemoved = FALSE;
	BOOL bIsInserted = FALSE;
	HANDLE hHandle = NULL;
	DEVINFO stMyDev = {0};
	CHAR csLastDevNameList[1024] = {0};
	CHAR csNewDevNameList[1024] = {0};
	LPSTR lpDevName1 = NULL;
	LPSTR lpDevName2 = NULL;
	...
	/* 还有余留事件没有返回，每次返回一个设备名 */
	if (g_ulRemovedNameLen > 1)
	{
		if (*pulDevNameLen >= g_ulRemovedNameLen)
		{
			strcpy(szDevName, g_csRemovedNameList);
			g_ulRemovedNameLen -= (strlen(szDevName) + 1);
			if (g_ulRemovedNameLen > 1)
			{
				memcpy(g_csRemovedNameList, g_csRemovedNameList + (strlen(szDevName)+1), g_ulRemovedNameLen);
				memset(g_csRemovedNameList + g_ulRemovedNameLen, 0, 512 - g_ulRemovedNameLen);
			}
			else
			{
				memset(g_csRemovedNameList, 0, 512);
				g_ulRemovedNameLen = 0;
			}
			*pulDevNameLen = strlen(szDevName);
			*pulEvent = 2;

			//memcpy_s(szDevName, *pulDevNameLen, g_csRemovedNameList, g_ulRemovedNameLen);
			//*pulDevNameLen = g_ulRemovedNameLen;
			//*pulEvent = 2;
			//memset(g_csRemovedNameList, 0, 512);
			//g_ulRemovedNameLen = 0;
			return SAR_OK;
	...
	if (g_ulInsertedNameLen > 1)
	{
		if (*pulDevNameLen >= g_ulInsertedNameLen)
		{
			strcpy(szDevName, g_csInsertedNameList);
			g_ulInsertedNameLen -= (strlen(szDevName) + 1);
			if (g_ulInsertedNameLen > 1)
			{
				memcpy(g_csInsertedNameList, g_csInsertedNameList + (strlen(szDevName)+1), g_ulInsertedNameLen);
				memset(g_csInsertedNameList + g_ulInsertedNameLen, 0, 512 - g_ulInsertedNameLen);
			}
			else
			{
				memset(g_csInsertedNameList, 0, 512);
				g_ulInsertedNameLen = 0;
			}
			*pulDevNameLen = strlen(szDevName);
			*pulEvent = 1;

			//memcpy_s(szDevName, *pulDevNameLen, g_csInsertedNameList, g_ulInsertedNameLen);
			//*pulDevNameLen = g_ulInsertedNameLen;
			//*pulEvent = 1;
			//memset(g_csInsertedNameList, 0, 512);
			//g_ulInsertedNameLen = 0;
			return SAR_OK;
	...
	/* 先枚举当前系统中已有的设备名列表 */
	P11_EnumDev(TRUE, csLastDevNameList, &ulLastNameListLen);
	
	/* 然后循环检查设备列表 */
	while (1)
	{	
		/* 重置变量 */
		ulNewNameListLen = 1024;
		g_ulRemovedNameLen = 0;
		g_ulInsertedNameLen = 0;
		memset(csNewDevNameList, 0, 1024);
		...
		/* 获取新的设备列表 */
		ulRes = P11_EnumDev(TRUE, csNewDevNameList, &ulNewNameListLen);
		if (SAR_OK != ulRes)
		{
			continue;
		}
		...
		/* 先判断是否有拔除的设备 */
		lpDevName1 = csLastDevNameList;
		while (lpDevName1 && strlen(lpDevName1) > 0)
		{
			lpDevName2 = csNewDevNameList;
			while (lpDevName2 && strlen(lpDevName2) > 0)
			{
				if (strcmp(lpDevName1, lpDevName2) == 0)
				{
					break;
				}
				lpDevName2 += (strlen(lpDevName2) + 1);
			}

			/* lpDevName1被移除 */
			if (NULL == lpDevName2 || strlen(lpDevName2) == 0)
			{
				memcpy_s(g_csRemovedNameList, 512-g_ulRemovedNameLen, lpDevName1, strlen(lpDevName1));
				g_ulRemovedNameLen += (strlen(lpDevName1) + 1);
			}

			/* 检查下一个 */
			lpDevName1 += (strlen(lpDevName1) + 1);
		}
		g_ulRemovedNameLen += 1;
				
		/* 再判断是否有新设备接入 */
		lpDevName1 = csNewDevNameList;
		while (lpDevName1 && strlen(lpDevName1) > 0)
		{
			lpDevName2 = csLastDevNameList;
			while (lpDevName2 && strlen(lpDevName2) > 0)
			{
				if (strcmp(lpDevName1, lpDevName2) == 0)
				{
			...
			if (NULL == lpDevName2 || strlen(lpDevName2) == 0)
			{
				memcpy_s(g_csInsertedNameList, 512-g_ulInsertedNameLen, lpDevName1, strlen(lpDevName1));
				g_ulInsertedNameLen += (strlen(lpDevName1) + 1);
			}

			/* 检查下一个 */
			lpDevName1 += (strlen(lpDevName1) + 1);
		}
		g_ulInsertedNameLen += 1;
		
		/* 有事件发生 */
		if (g_ulRemovedNameLen > 1 || g_ulInsertedNameLen > 1)
		{
			/* 保存状态 */
			memcpy_s(csLastDevNameList, 1024, csNewDevNameList, ulNewNameListLen);
			ulLastNameListLen = ulNewNameListLen;
	...
	{
		bExistDevice = TRUE;
		P11_GetDevInfo(hHandle, &stMyDev);
    ...
    if (bPresent)
    {
    	for(i = 0; i < MAX_DEV_CNT; i++)
    	{
            ...
            if(SAR_OK == ulRet)
            {
                ...
                ulRet = P11_GetDevInfo(hDev, &stMyDev);
				...
				memcpy(acTmp, stMyDev.SerialNumber, 32);
	...
	for(i = 0; i < MAX_DEV_CNT; i++)
	{
        *phDev = NULL;
        ...
        if (SAR_OK == ulRet)
        {
            ...
            ulRet = P11_GetDevInfo(*phDev, &stMyDev);
            if (SAR_OK != ulRet)
            {
                LOG_ErrNo(ulRet);
                return ulRet;
    ...
    ULONG ulPinType, CHAR *pcOldPin, CHAR *pcNewPin, ULONG *pulRetryCount)
{
    ...
    /* 校验PIN码 */
	ulRet = P11_VerifyPIN(hApplication, ulPinType, pcOldPin, pulRetryCount);
	if (SAR_OK != ulRet)
	{
		return ulRet;
	}
    ...
    ULONG ulPinType, CHAR *pcPin, ULONG *pulRetryCount)
{
    ...
    /* 产生8位的随机数 */
	ulRet = P11_GenRandom(phAppHandle->phDev, aucRandom, 8);
	if(ulRet != SAR_OK)
	{
        LOG_ErrNo(ulRet);
		return ulRet;
    ...
    CHAR *pcAdminPIN, CHAR *pcNewUserPIN, ULONG *pulRetryCount)
{
    ...
    /* 先校验管理员PIN码 */
	ulRet = P11_VerifyPIN(hApplication, KT_SOPIN, pcAdminPIN, pulRetryCount);
	if (SAR_OK != ulRet)
	{
		return ulRet;
	}
...
ULONG P11_CreateApplication(DEVHANDLE hDev, LPSTR szAppName,
    ...
    ULONG ulCreateFileRights, HAPPLICATION *phApplication)
{
	...
	ulRet = P11_OpenApplication(hDev, szAppName, phApplication);
    ...
    CHAR *pcAppName, HAPPLICATION *phApplication)
{
    ...
    phAppHandle = (APPHANDLE_S *)SYNO_calloc(1, sizeof(APPHANDLE_S));
    if (NULL == phAppHandle)
    {
    ...
    CHAR *pcFileList, ULONG *pulSize)
{
    ...
    /* 参数检查 */
	if ((NULL == phAppHandle) || (NULL == pulSize))
	{
        LOG_ErrNo(SAR_INVALIDPARAMERR);
		return SAR_INVALIDPARAMERR;
	}
...
ULONG P11_ReadFile(HAPPLICATION hApplication, LPSTR szFileName,
    ULONG ulOffset, ULONG ulSize, BYTE *pbOutData, ULONG *pulOutLen)
{
	...
	ulRet = P11_GetFileInfo(hApplication, szFileName, (FILEATTRIBUTE *)&stFAttri);
	if(ulRet != SAR_OK)
	{
		return ulRet;
	}
    ...
    LPSTR szContainerName, HCONTAINER *phContainer)
{
    ...
    pContainer = (CONTAINERHANDLE_S *)SYNO_calloc(1, sizeof(CONTAINERHANDLE_S));
    if (NULL == pContainer)
    {
        LOG_ErrNo(SAR_MEMORYERR);
		return SAR_MEMORYERR;
...
ULONG P11_GetContainerInfo(HCONTAINER hContainer, 
						   ULONG *pulContainerType)
{
	return P11_GetContainerType(hContainer, pulContainerType);
}
							 ...
							 LPCONTAINERINFOEX lpContainerInfoEx)
{
	...
	UCHAR aucOutBuf[MAX_BUF] = {0};
    ULONG ulCmdLen = 0;
	ULONG ulOutLen = sizeof(ContainerInfoEx);

    /* 参数检查 */
	if ((NULL == hApplication)
		|| (NULL == szContainerName)
        || (NULL == lpContainerInfoEx))
	{
        LOG_ErrNo(SAR_INVALIDPARAMERR);
		return SAR_INVALIDPARAMERR;
	}
	...
	memcpy(aucCmdBuf, &phAppHandle->usAppID, 2);
    ulCmdLen = 2;
	memcpy(aucCmdBuf + ulCmdLen, szContainerName, strlen(szContainerName));
    ulCmdLen += strlen(szContainerName);

    ulRet = CMD_Transmit(phAppHandle->phDev, CMD_GET_CONTAINERINFOEX, 0x00, 0x00,
        aucCmdBuf, ulCmdLen, aucOutBuf, &ulOutLen);
    if (SAR_OK == ulRet)
    {
		memcpy(lpContainerInfoEx, aucOutBuf, sizeof(ContainerInfoEx));
    }
...
ULONG P11_GenRSAKeyPair(HCONTAINER hContainer, 
						UCHAR ucUsage,
						ULONG ulBitsLen, 
						PRSAPUBLICKEYBLOB pBlob)
{
    ULONG ulRet = SAR_OK;
    ...
    ulRet = CMD_Transmit(phContainer->phDev, CMD_GEN_RSAKEYPAIR, ucUsage, 0x00,
        aucCmdBuf, ulCmdLen, aucOutBuf, &ulOutLen);
    if(ulRet != SAR_OK)
    {
		return ulRet;
...
ULONG P11_ImportPlainRSAKeyPair(HCONTAINER hContainer, 
								UCHAR ucUsage, 
								PRSAPRIVATEKEYBLOB pBlob)
{
    ...
    ulRet = CMD_Transmit(phContainer->phDev, CMD_IMPORT_PLAINRSAKEY, ucUsage, 0x00,
        aucCmdBuf, ulCmdLen, NULL, NULL);
...
ULONG P11_ImportPlainRSAPubKey(HCONTAINER hContainer, UCHAR ucUsage, PRSAPUBLICKEYBLOB pBlob)
{
	...
	ulRet = CMD_Transmit(phContainer->phDev, CMD_IMPORT_RSAPUBKEY, ucUsage, 0x00,
        aucCmdBuf, ulCmdLen, NULL, NULL);

	return ulRet;
}

ULONG P11_ImportPlainRSAPriKey(HCONTAINER hContainer, 
							   UCHAR ucUsage, 
							   PRSAPRIVATEKEYBLOB pBlob)
{
	ULONG ulRet = SAR_OK;
	...
	ulRet = CMD_Transmit(phContainer->phDev, CMD_IMPORT_RSAPRIKEY, ucUsage, 0x00,
        aucCmdBuf, ulCmdLen, NULL, NULL);
    ...
    BYTE *pbData, ULONG ulDataLen, BYTE *pbSignature, ULONG *pulSignLen)
{
	...
	if (ulDataLen > (256-11))		//该函数不对数据做Hash，初步假定为2048RSA
	{
		return SAR_INDATALENERR;
	}
...
ULONG P11_RSAVerify(DEVHANDLE hDev, PRSAPUBLICKEYBLOB pRSAPubKeyBlob,
	...
	PRSAPUBLICKEYBLOB pPubKey, BYTE *pbData, ULONG *pulDataLen, HANDLE *phSessionKey)
{
    ...
    *pulDataLen = ulByteLen;
    ...
    pKeyHandle = (KEYHANDLE_S *)SYNO_calloc(1, sizeof(KEYHANDLE_S));
    if (pKeyHandle == NULL)
    {
        LOG_ErrNo(SAR_MEMORYERR);
		return SAR_MEMORYERR;
    ...
    RSAPUBLICKEYBLOB *pPubKey, BYTE *pbData, ULONG *pulDataLen)
{
    ...
    *pulDataLen = ulByteLen;
    ...
    ulOutLen = ulByteLen + 2;
    ulRet = CMD_Transmit(pKeyHandle->phDev, CMD_RSA_EXPORTSESSIONKEYEX, 0x00, 0x00,
        aucCmdBuf, ulCmdLen, aucOutBuf, &ulOutLen);
    if(ulRet != SAR_OK)
	{
		return ulRet;
	...
	memcpy(pbData, aucOutBuf, ulByteLen);
	*pulDataLen = ulByteLen;
	...
	BYTE *pbInput, ULONG ulInputLen, BYTE *pbOutput, ULONG *pulOutputLen)
{
	...
	if ((bEncrypt && (ulInputLen > (256-11)))
		|| (!bEncrypt && (ulInputLen > 256)))
	{
        LOG_ErrNo(SAR_INVALIDPARAMERR);
		return SAR_INVALIDPARAMERR;
	}
	...
	phContainer = (CONTAINERHANDLE_S *)hContainer;
	usFlag = bEncrypt ? 1 : 0;
	ucUseSignKeyPair = bUseSignKeyPair ? 1 : 0;
	ucUsePubKey = bUsePubKey ? 1 : 0;

	// 原文数据
	memcpy(aucCmdBuf, pbInput, ulInputLen);
    ulCmdLen += ulInputLen;
	// App ID
	memcpy(aucCmdBuf + ulCmdLen, &phContainer->AppID, 2);
    ulCmdLen += 2;
	// Container ID
	memcpy(aucCmdBuf + ulCmdLen, &phContainer->ContainerID, 2);
    ulCmdLen += 2;
	// Flag(1:加密操作；0:解密操作)
	memcpy(aucCmdBuf + ulCmdLen, (UCHAR *)&usFlag, 2);
    ulCmdLen += 2;
	
    ulRet = CMD_Transmit(phContainer->phDev, CMD_RSAEXTKEY_OPT, ucUseSignKeyPair, ucUsePubKey,
        aucCmdBuf, ulCmdLen, pbOutput, pulOutputLen);
    ...
    BYTE *pbID, ULONG ulIDLen, HANDLE *phAgreementHandle)
{
    ...
    pAgreementHandle = (KEYHANDLE_S *)SYNO_calloc(1, sizeof(KEYHANDLE_S));
...
ULONG P11_ExportPublicKey(HCONTAINER hContainer, BOOL bSignFlag,
    BYTE* pbBlob, ULONG* pulBlobLen)
{
	...
	P11_GetContainerType(hContainer, &ulContainerType);
...
ULONG P11_ImportPublicKey(HCONTAINER hContainer, BOOL bSignFlag, 
    ...
    ULONG ulAlgId, BYTE *pbWrapedData, ULONG ulWrapedLen, HANDLE *phKey)
{
	...
	// 获取容器类型
	ulRet = P11_GetContainerType(hContainer, &ulConType);
...
ULONG P11_GenSymmKey(DEVHANDLE hDev, ULONG ulAlgID, HANDLE *phKey)
{
    ...
    ulOutLen = 2;
    ulRet = CMD_Transmit(hDev, CMD_GENERATE_SYMMKEY, 0x00, 0x00,
        aucCmdBuf, ulCmdLen, aucOutBuf, &ulOutLen);
	if(ulRet != SAR_OK)
	{
		return ulRet;
...
ULONG P11_EncryptInit(HANDLE hKey, ULONG ulAlgID, BLOCKCIPHERPARAM EncryptParam)
{
	...
	case SGD_AES128_CBC:
	case SGD_AES128_ECB:
		if (SGD_AES128 == pKeyHandle->ulAlgID) bAlgIdIsOk = TRUE;
		else bAlgIdIsOk = FALSE;
		break;
	case SGD_AES192_CBC:
	case SGD_AES192_ECB:
		if (SGD_AES192 == pKeyHandle->ulAlgID) bAlgIdIsOk = TRUE;
		else bAlgIdIsOk = FALSE;
		break;
	case SGD_AES256_CBC:
	case SGD_AES256_ECB:
		if (SGD_AES256 == pKeyHandle->ulAlgID) bAlgIdIsOk = TRUE;
		else bAlgIdIsOk = FALSE;
		break;
	case SGD_DES_CBC:
	case SGD_DES_ECB:
		if (SGD_DES == pKeyHandle->ulAlgID) bAlgIdIsOk = TRUE;
		else bAlgIdIsOk = FALSE;
		break;
	case SGD_DES3_CBC:
	case SGD_DES3_ECB:
		if (SGD_DES3 == pKeyHandle->ulAlgID) bAlgIdIsOk = TRUE;
		else bAlgIdIsOk = FALSE;
		break;
	default:
		bAlgIdIsOk = FALSE;
    ...
    BYTE *pbData, ULONG ulDataLen, BYTE *pbEncryptedData, ULONG *pulEncryptedLen)
{
	...
	if (g_ulPaddingType)
	{
		...
		PKCS7_AddPadding(aucPad, ulPadByte);
	}
	else
	{
		if (ulDataLen % 16)
	...
	{
		*pulEncryptedLen = ulDataLen + ulPadByte;
		return SAR_OK;
	}
	if (*pulEncryptedLen < ulDataLen + ulPadByte)
	{
		return SAR_INDATALENERR;
	}
	...
	if (ulPadByte > 0)
	{
		memcpy(aucCmdBuf + ulCmdLen, aucPad, ulPadByte);
		ulCmdLen += ulPadByte;
	}
    ...
    BYTE *pbData, ULONG ulDataLen, BYTE *pbEncryptedData, ULONG *pulEncryptedLen)
{
	...
	if (g_ulPaddingType)
	{
		if (ulDataLen % 16)
		{
			g_ulPadLen = 16 - (ulDataLen % 16);
			PKCS7_AddPadding(g_aucPadByte, g_ulPadLen);
		}
		else
		{
			g_ulPadLen = 16;
			memset(g_aucPadByte, 0x10, 0x10);
		}
	}
	else
	{
	...
	/* 返回长度 */
	ulOutLen = ulDataLen + (g_ulPadLen==0x10 ? 0 : g_ulPadLen);
	if (NULL == pbEncryptedData)
	{
		*pulEncryptedLen = ulOutLen;
		return SAR_OK;
	}
	...
	memcpy(aucCmdBuf + ulCmdLen, pbData, ulDataLen);
	ulCmdLen += ulDataLen;
	if (g_ulPadLen > 0 && g_ulPadLen != 0x10)
	{
		memcpy(aucCmdBuf + ulCmdLen, g_aucPadByte, g_ulPadLen);
		ulCmdLen += g_ulPadLen;
	}
    ...
    BYTE *pbEncryptedData, ULONG *pulEncryptedDataLen)
{
	...
	/* 补码长度*/
    if (g_ulPaddingType && (g_ulPadLen == 0x10))
	{
		ulOutLen = 0x10;
	}
	else
	...
	if (*pulEncryptedDataLen < ulOutLen)
	{
		return SAR_INVALIDPARAMERR;
	}
    ...
    BYTE *pbEncryptedData, ULONG ulEncryptedLen, BYTE *pbData, ULONG *pulDataLen)
{
	...
	*pulDataLen = ulEncryptedLen;
	...
	if (g_ulPaddingType)
	{
		PKCS7_RemovePadding(pbData, pulDataLen);
		g_ulPaddingType = 0;
	}
    ...
    BYTE *pbDecryptedData, ULONG *pulDecryptedDataLen)
{
	...
	if (g_ulPaddingType)
	{
		PKCS7_RemovePadding(pbDecryptedData, pulDecryptedDataLen);
		g_ulPaddingType = 0;
	}
    ...
    ECCPUBLICKEYBLOB *pPubKey, BYTE *pbID, ULONG ulIDLen, HANDLE *phHash)
{
    ...
    pHashHandle = (KEYHANDLE_S *)SYNO_calloc(1, sizeof(KEYHANDLE_S));
    if (NULL == pHashHandle)
    {
        LOG_ErrNo(SAR_MEMORYERR);
		return SAR_MEMORYERR;
...
ULONG P11_MacInit(HANDLE hKey, BLOCKCIPHERPARAM *pMacParam, HANDLE *phMac)
{
    ...
    pMacHandle = (KEYHANDLE_S *)SYNO_calloc(1, sizeof(KEYHANDLE_S));
    if (NULL == pMacHandle)
    {
        LOG_ErrNo(SAR_MEMORYERR);
		return SAR_MEMORYERR;
...
BOOL P11_ExterAuth(HANDLE hHandle, UCHAR *pucKey, ULONG ulLen)
{
	...
	ulRet = P11_GenRandom(hHandle, bData, 8);
    if (SAR_OK != ulRet)
    {
        return FALSE;
    }
    ...
    ulRet = P11_DevAuth(hHandle, pOutData, 16);
    if (SAR_OK != ulRet)
    {
        return FALSE;
    }
    ...
    BYTE *pbData, ULONG ulDataLen, BYTE *pbEncryptedData, ULONG *pulEncryptedLen)
{
	...
	//{
	//	BYTE btPadded[16] = {0};
	//	ULONG ulPaddedLen = 16 - ulDataLen % 16;
	//	lpPaddedData =  (BYTE*)SYNO_malloc(ulDataLen + ulPaddedLen);
	//	PKCS7_AddPadding(btPadded, ulPaddedLen);
	//	memcpy(lpPaddedData, pbData, ulDataLen);
	//	memcpy(lpPaddedData + ulDataLen, btPadded, ulPaddedLen);
	//	ulDataLen += ulPaddedLen;
	//	lpInData = lpPaddedData;
	//}
	if (ulDataLen % 16)
	{
        return SAR_INDATALENERR;
	...
	/* 采用数据通道加密 */
    ulRet = DataInterface(hDev, lpInData, ulDataLen, pbEncryptedData, pulEncryptedLen, ENC);
	
	SYNO_free(lpPaddedData);
	return ulRet;
}
    ...
    BYTE *pbData, ULONG ulDataLen, BYTE *pbDecryptedData, ULONG *pulDecryptedLen)
{
	...
	//PKCS7_RemovePadding(pbDecryptedData, pulDecryptedLen);
...
ULONG P11_ExtKeyOperation(HCONTAINER hContainer,BYTE AlgID, BYTE KeyType, BYTE OpType,
	BYTE* pbInput, ULONG ulInputLen, BYTE * pbOutput, ULONG* pulOutputLen)
{
    return 0;
}

ULONG P11_FormatCD( HANDLE hHandle,
      char* psFileName)
{
	return SAR_OK;
}

ULONG P11_ImportRSAKey(HCONTAINER hContainer, RSAPRIVATEKEYBLOB *pRSAPriKey, BYTE KeyType)
{
    return SAR_OK;
}

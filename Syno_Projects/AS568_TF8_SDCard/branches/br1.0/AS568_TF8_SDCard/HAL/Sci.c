				...
				FP_SCI_POWER_CTRL fpSciPowerCtrl)
{
	...
	// to select Pins - GPIOA28/29/30/31 for SCI signals
...
SCI_ERR SCI_ActiveOnly(UINT8 nType, UINT32 nFeq, UINT32 nTimeout)
{
	...
	if(SCI_CheckCardIn(nTimeout) != SCI_OK) return SCI_CARD_ABSENT;
 ...
 * ModifyRecord:
 * ********************************************************************************/
SCI_ERR SCI_ResumeClock(UINT8 StartDly, UINT32 nTimeout)
{
	UINT32 nTimeCnt = 0;

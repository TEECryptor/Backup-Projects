 ...
 */
/*
 * RCSID:       $Id: internal.h,v 1.3 2000/01/31 18:09:02 lbe Exp $
 * Source:      $Source: /usr/cvsroot/pkcs11/libgpkcs11/internal.h,v $
 * Last Delta:  $Date: 2000/01/31 18:09:02 $ $Revision: 1.3 $ $Author: lbe $
...
extern CK_BBOOL g_ucUseOpenssl;
extern CK_CHAR g_acSerialNumber01[16];
extern CK_CHAR g_acSerialNumber02[16];
extern CK_CHAR g_acSerialNumber03[16];

CK_CHAR_PTR g_pcSerialNumber;

#define CK_I_SES_CONTAINER_COUNT    20  /* 应用最大容器个数 */
...
typedef CK_CALLBACK_FUNCTION(CK_RV, CIP_InitToken)(
  CK_I_SLOT_DATA_PTR slot_data,/* data of the token to be initialized */
...
typedef struct ck_i_token_data_st {
  ...
  CK_ULONG symalg_flags;
  CK_ULONG asymalg_flags;
  CK_ULONG hashalg_flags;
} CK_I_TOKEN_DATA;
 ...
 */
CK_DECLARE_I_FUNCTION(CK_RV, CI_RegisterSlot)(
 CK_ULONG slotID,
...
struct ck_i_obj_st {
  ...
  CK_ULONG obj_id;
  CK_VOID_PTR ObjHandle;    /* 对象句柄 */
  CK_VOID_PTR CtrHandle;    /* 对象所属容器句柄 */
};
...
struct ck_i_session_data_st {
  ...
  CK_VOID_PTR AppHandle;               /* 这里假设设备里的一个应用可以多次打开 */
  CK_VOID_PTR MainCtrHandle;              /* 设备主容器句柄,存储设备自己的公私钥对 */
  CK_VOID_PTR aCtrHandle[CK_I_SES_CONTAINER_COUNT]; /* 用于存放应用内容器句柄 */
  CK_ULONG CtrCount;                   /* 容器计数 */
};

...
//--------------------------------------------------------------
JPEG_CORE_PROPERTIES CJpgManager::s_Image;
//--------------------------------------------------------------
...
//	初始化ijl库
//--------------------------------------------------
BOOL CJpgManager::InitIjlLibrary()
{
	if( ijlInit( &s_Image ) != IJL_OK )
	{
		::MessageBox(NULL, "初始化Ijl库失败！", "错误", MB_OK);
		return false;
	}
	else return true;
}
...
//	释放ijl库
//--------------------------------------------------
void CJpgManager::FreeIjlLibrary()
{
	if( ijlFree( &s_Image ) != IJL_OK )
	{
		::MessageBox(NULL, "释放Ijl库失败！", "错误", MB_OK);
	}
}
//--------------------------------------------------
//	加载
//--------------------------------------------------
BOOL CJpgManager::LoadImageFromBuff(BYTE* pJpgData, int iJpgDataSize, BYTE*& pData, int &iWidth, int &iHeight)
{	
	BYTE* imageData;

	//jpg文件的数据buff和大小
	s_Image.JPGFile = "no name";
	s_Image.JPGBytes = pJpgData;
	s_Image.JPGSizeBytes = iJpgDataSize;

	//从buff里读取图片属性数据
	IJLERR ijlErr;
	if( (ijlErr = ijlRead( &s_Image, IJL_JBUFF_READPARAMS )) != IJL_OK )
	{
		::MessageBox(NULL, "读取jpg文件属性错误！", "错误", MB_OK);
		return false;
	}

	switch(s_Image.JPGChannels)
	{
	case 1:
	  s_Image.JPGColor    = IJL_G;
	  s_Image.DIBChannels = 3;
	  s_Image.DIBColor    = IJL_BGR;
	  break;
	case 3:
	  s_Image.JPGColor    = IJL_YCBCR;
	  s_Image.DIBChannels = 3;
	  s_Image.DIBColor    = IJL_BGR;
	  break;
	case 4:
	  s_Image.JPGColor    = IJL_YCBCRA_FPX;
	  s_Image.DIBChannels = 4;
	  s_Image.DIBColor    = IJL_RGBA_FPX;
	  break;
	default:
	  // This catches everything else, but no
	  // color twist will be performed by the IJL.
	  s_Image.DIBColor = (IJL_COLOR)IJL_OTHER;
	  s_Image.JPGColor = (IJL_COLOR)IJL_OTHER;
	  s_Image.DIBChannels = s_Image.JPGChannels;
	  break;
	}

	//图片属性
	s_Image.DIBWidth    = s_Image.JPGWidth;
	s_Image.DIBHeight   = s_Image.JPGHeight;
	s_Image.DIBPadBytes = IJL_DIB_PAD_BYTES(s_Image.DIBWidth, s_Image.DIBChannels);

	int iImageSize = (s_Image.DIBWidth * s_Image.DIBChannels + s_Image.DIBPadBytes) * s_Image.DIBHeight;

	imageData = new BYTE[ iImageSize ];
	if( imageData == NULL )
	{
		::MessageBox(NULL, "分配内存空间错误！", "错误", MB_OK);
		return false;
	}

	s_Image.DIBBytes = imageData;

	//读取图片数据
	if( ijlRead( &s_Image, IJL_JBUFF_READWHOLEIMAGE ) != IJL_OK )
	{
		::MessageBox(NULL, "读取jpg数据错误！", "错误", MB_OK);
		delete[] imageData;
		imageData = NULL;
	}

	pData = imageData;
	iWidth	 = s_Image.DIBWidth;
	iHeight	 = s_Image.DIBHeight;
...
BOOL CJpgManager::jpgReadBuff(BYTE* pJpgData, int iJpgDataSize)
{
	...
	static JPEG_CORE_PROPERTIES image;
	ZeroMemory( &image, sizeof( JPEG_CORE_PROPERTIES ) );
	BYTE* imageData;

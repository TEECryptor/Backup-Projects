...
应用程序向导已为您创建了此 VGSATL2 项目，作为编写动态链接库 (DLL) 的起点。
...
VGSATL2.idl
    ...
    此文件将由 MIDL 编译器进行处理以生成：
        C++ 接口定义和 GUID 声明 (VGSATL2.h)
        GUID 定义                                (VGSATL2_i.c)
        类型库                                  (VGSATL2.tlb)
        封送处理代码                                 （VGSATL2_p.c 和 dlldata.c）

VGSATL2.h
    此文件包含 VGSATL2.idl 中定义的项目的 C++ 接口定义和 GUID 声明。它将在编译过程中由 MIDL 重新生成。
...
StdAfx.h, StdAfx.cpp
    这些文件用于生成名为 VGSATL2.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
...
VGSATL2ps.vcproj
	...
	dlldata.c、VGSATL2_i.c 和 VGSATL2_p.c，这些文件是

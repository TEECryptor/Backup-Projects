...
)
{
    ...
    * No sense opening a session in the void does now? B-) */
    ...
    if (g_ucUseOpenssl)
    {
        ...
        if(CK_I_Method_P == NULL_PTR)
        {
            rv = CKR_OK;
        }
        else
        {
            rv = CK_I_Method_P (new_session);
        }
    }
    else
    {
        rv = ICA_TOK_OpenSession(new_session);
    }
      ...
      )
{
    ...
    {
        rv = ICA_TOK_CloseSession(session_data);
        if(rv != CKR_OK)
        {
            return rv;
        }
...
)
{
    ...
    {
        rv = ICA_TOK_Login(session_data, userType, pPin, ulPinLen);
        if(rv != CKR_OK)
        {
            CI_LogEntry("C_Login", "token call of Login failed", rv, 0);
            return rv;
...
)
{
    ...
    {
        rv = ICA_TOK_Logout(session_data);
        if(rv != CKR_OK)
        {
            return rv;
        }

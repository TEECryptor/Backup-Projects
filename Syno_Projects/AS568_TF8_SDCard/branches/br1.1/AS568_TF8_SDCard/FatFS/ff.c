...
/*-----------------------------------------------------------------------*/
/* FAT handling - Find a continuous cluster chain for 'cnt' count        */
/*-----------------------------------------------------------------------*/
static
DWORD find_continuous_chain (/* 0:No suitable cluster for continuous, 1:Internal error, 0xFFFFFFFF:Disk error, >=2:New cluster# */
    FATFS* fs,               /* File system object */
    DWORD clst,              /* Cluster# to stretch. 0 means for a new chain. */
    DWORD cnt                /* The chain size */
)
{
	DWORD cs = 0, ncl = 0, scl = 0, found = 0, start = 0;
	FRESULT res;
	...
	for (;;) {
		ncl++;							/* Next cluster */
		if (ncl >= fs->n_fatent) {		/* Check wrap around */
		    return 0;	/* No enough free cluster */
		}
		cs = get_fat(fs, ncl);			/* Get the cluster status */
		if (cs == 0) {				    /* Found a free cluster */
            if (found++ >= cnt) {       /* Found 'cnt' count continuous clousters */
                break;
            }
        }
        else {
    ...
    start = scl + 1;
	res = put_fat(fs, start, 0x0FFFFFFF);	/* Mark the new cluster "last link" */
	if (res == FR_OK && clst != 0) {
		res = put_fat(fs, clst, start);	/* Link it to the previous one if needed */
	}
	if (res == FR_OK) {
		fs->last_clust = start;			/* Update FSINFO */
		if (fs->free_clust != 0xFFFFFFFF) {
			fs->free_clust--;
			fs->fsi_flag |= 1;
		}
	...
	return start;		/* Return the start cluster for the chain */
}
#endif /* !_FS_READONLY */
...
static
BYTE check_fs1 (	/* 0:FAT boor sector, 1:Valid boor sector but not FAT, 2:Not a boot sector, 3:Disk error */
	FATFS* fs,	/* File system object */
...
)
{
	...
	int vol;
	DSTATUS stat;
	DWORD bsect, fasize, tsect, sysect, nclst, szbfat, br[4],br1[4];
	WORD nrsv;
	FATFS *fs;
	UINT i;
	//WORD MBRFlag = 1;
...
//	if (fmt) return FR_NO_FILESYSTEM;		/* No FAT volume is found */
	...
	//mbr
	bsect = 0;
	fmt = check_fs1(fs, bsect);					/* Load sector 0 and check if it is an FAT boot sector as SFD */
	if (fmt == 3) return FR_DISK_ERR;		/* An error occured in the disk I/O layer */
	if (fmt == 2) return FR_NO_FILESYSTEM;		/* No FAT volume is found */
	if (fmt == 1 || (!fmt && (LD2PT(vol)))) {	/* Not an FAT boot sector or forced partition number */
		for (i = 0; i < 4; i++) {			/* Get partition offset */
			pt = fs->win + MBR_Table + i * SZ_PTE;
			if((pt[0] == 0x00)||(pt[0] == 0x80))
			{
				br1[i] = 0xaa;
				br[i] = pt[4] ? LD_DWORD(&pt[8]) : 0;
			}
		...
		if (i) i--;
		do {
			if(br1[i] == 0xaa)								/* Find an FAT volume */
			{
				bsect = br[i];
				fmt = bsect ? check_fs(fs, bsect) : 2;	/* Check the partition */
			}
		...
		if (ofs) {
			...
			if (ifptr > 0 &&
				...
				if (clst == 0) {						/* If no cluster chain, create a new chain */
                    ...
                    clst = find_continuous_chain(fp->fs, 0, ofs / bcs + 1);     /*  To find a contiunous chain. Youfang*/
					if (clst == 1) ABORT(fp->fs, FR_INT_ERR);
					if (clst == 0xFFFFFFFF) ABORT(fp->fs, FR_DISK_ERR);
					fp->sclust = clst;
				}

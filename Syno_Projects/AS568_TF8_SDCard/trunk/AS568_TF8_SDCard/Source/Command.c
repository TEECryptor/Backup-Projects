...
SM3_CONTEXT     gSm3Context = {0};
SHA1_CONTEXT    gSHA1Context = {0};
SHA256_CONTEXT  gSHA256Context = {0}; 
MD5_CONTEXT     gMD5Context = {0}; 
//
...
{  		
	...
	{         	   
        ...
        case CMD_GET_CONTAINERINFOEX:
            CMD_GetContainerInfoEx();break;
		case CMD_CONTYPE:
        ...
        case CMD_IMPORT_RSAPUBKEY:
            CMD_ImportRSAPubKey();break;
        case CMD_IMPORT_RSAPRIKEY:
            CMD_ImportRSAPriKey();break;                  
        ...
        case CMD_GENERATE_SYMMKEY:
            CMD_GenerateSymmKey();break;
...
/***************************************************************************
* Subroutine:	CMD_GetContainerInfoEx()
* Function:		Get container information for P11 by container name
* Author:   Syno Common	
* Date:		2015.11.24
* Version:  V1.0
* ModifyRecord:
* ************************************************************************/
void CMD_GetContainerInfoEx()
{
	UINT16 nRet = SAR_OK;
    UINT16 AppID = 0;
	UINT32 nKeyBits = 0;
	...
	if (nRet == RT_OK)
	{
		ConInfoEx.type = Node.ContainerType;
	}	
	else
	{			 
		ConInfoEx.type = 0;
	...
	if (RT_OK == nRet)
	{
        ConInfoEx.signKeyPair = 1;
		if (ConInfoEx.type == RSA_CON)
		{
			ConInfoEx.signKeyBits = 8 * ((Node.FileSize-6) / 4.5);
		}
		else if (ConInfoEx.type == ECC_CON)
		{
			ConInfoEx.signKeyBits = 8 * (Node.FileSize / 3);
		}
	}
    else
    {   
	    nRet = FS_SelectFileByID(SIGNPUBKEYID); 
	    nRet |= FS_ReadFileNodeInfo((UINT8*)&Node);
	    if (RT_OK == nRet)
        {                      
            ConInfoEx.hasVerfiyPubKey = 1;
    		if (ConInfoEx.type == RSA_CON)
    		{
    			ConInfoEx.signKeyBits = 8 * (Node.FileSize - 6);
    		}
    		else if (ConInfoEx.type == ECC_CON)
    		{
    			ConInfoEx.signKeyBits = 8 * (Node.FileSize / 2);
    		}
        }
	    nRet = FS_SelectFileByID(SIGNPRIKEYID); 
	    nRet |= FS_ReadFileNodeInfo((UINT8*)&Node);
	    if (RT_OK == nRet)
        {                           
            ConInfoEx.hasSignPriKey = 1;
    		if (ConInfoEx.type == RSA_CON)
    		{
    			ConInfoEx.signKeyBits = 8 * ((Node.FileSize-6) / 4.5);
    		}
    		...
    		{
    			ConInfoEx.signKeyBits = 8 * Node.FileSize;
    		}
        }
    }
	...
	if (RT_OK == nRet)
	{                   
        ConInfoEx.exchKeyPair = 1;
		if (ConInfoEx.type == RSA_CON)
		{					 
			ConInfoEx.exchKeyBits = 8 * ((Node.FileSize-6) / 4.5);	
		}
		else if (ConInfoEx.type == ECC_CON)
		{	
			ConInfoEx.exchKeyBits = 8 * (Node.FileSize / 3);
		}
	}
    else
    {    
	    nRet = FS_SelectFileByID(EXPUBKEYID); 
	    nRet |= FS_ReadFileNodeInfo((UINT8*)&Node);
	    if (RT_OK == nRet)
        {                      
            ConInfoEx.hasEncryptPubKey = 1;
    		if (ConInfoEx.type == RSA_CON)
    		{
    			ConInfoEx.exchKeyBits = 8 * (Node.FileSize - 6);
    		}
    		else if (ConInfoEx.type == ECC_CON)
    		{
    			ConInfoEx.exchKeyBits = 8 * (Node.FileSize / 2);
    		}
        }
	    nRet = FS_SelectFileByID(EXPRIKEYID); 
	    nRet |= FS_ReadFileNodeInfo((UINT8*)&Node);
	    if (RT_OK == nRet)
        {                           
            ConInfoEx.hasDecryptPriKey = 1;
    		if (ConInfoEx.type == RSA_CON)
    		{
    			ConInfoEx.exchKeyBits = 8 * ((Node.FileSize-6) / 4.5);
    		}
    		...
    		{
    			ConInfoEx.exchKeyBits = 8 * Node.FileSize;
    		}
        }
    }
	...
	if (RT_OK == nRet)
	{
		ConInfoEx.hasSignCert = 1;
	}
	else
	{
		ConInfoEx.hasSignCert = 0;
	}
	...
	if (RT_OK == nRet)
	{  
		ConInfoEx.hasExchCert = 1;
	}
	else
	{
		ConInfoEx.hasExchCert = 0;
	}

    memcpy(gApduResp.DATA, &ConInfoEx, sizeof(ConInfoEx));

    DBG_FUNC_END;
	SetCmdResult(SAR_OK, sizeof(ConInfoEx));
}
/***************************************************************************
...
/***************************************************************************
* Subroutine:	CMD_ImportRSAPubKey()
* Function:		Import rsa public key which be plain mode, only for P11
* Author:   Syno Common	
* Date:		2015.12.03
* Version:  V1.0
* ModifyRecord:
* ************************************************************************/
void CMD_ImportRSAPubKey(void)
{
    UINT16 AppID = 0;
    UINT16 ConID = 0;
    UINT16 nByteLen = 0;
	...
	nByteLen = keybitlen / 8;
	nPrimeLen = keybitlen / 16;
    keyPairLen = nByteLen*2 + nPrimeLen*5 + 6;
    ...
    //检查数据长度
	if ((2 + nByteLen + 4) != (gApduBody.LC - 4))
    {                                         
        DBG_ERR("Data length is wrong! gApduBody.LC = 0x%x", gApduBody.LC);
		SetCmdResult(SAR_INDATALENERR, 0);
		return;
	}
    memset(gApduBody.DATA + gApduBody.LC, 0, (keyPairLen+4-gApduBody.LC));
	...
	if(gApduHeader.P1 == TRUE)
	{
		memset((UINT8*)&Node, 0, sizeof(Node));
		Node.FileType = FT_KEY;
		Node.FileSize = keyPairLen;     //为了兼容其他操作函数，此处保存的数据还是按照KeyPair来保存，只是私钥数据全为0！
		Node.FCP.FileID = SIGNPUBKEYID;
		strcpy(Node.FCP.Name, "SignRSAPubKey");
	...
	else if(gApduHeader.P1 == FALSE)
	{
		...
		Node.FileType = FT_KEY;
		Node.FileSize = keyPairLen;     //为了兼容其他操作函数，此处保存的数据还是按照KeyPair来保存，只是私钥数据全为0！
		Node.FCP.FileID = EXPUBKEYID;
		strcpy(Node.FCP.Name, "ExtRSAPubKey");
    ...
    //写密钥数据，为了兼容其他操作函数，此处保存的数据还是按照KeyPair来保存，只是私钥数据全为0！
	nRet = FS_UpdateFile(gApduBody.DATA + 4, 0, keyPairLen);
	if (nRet != RT_OK)
	{                                     
        DBG_ERR("FS_UpdateFile() failed! nRet = 0x%x", nRet);
		SetCmdResult(SAR_WRITEFILEERR, 0);
...
/***************************************************************************
* Subroutine:	CMD_ImportRSAPriKey()
* Function:		Import rsa private key which be plain mode, only for P11
* Author:   Syno Common	
* Date:		2015.12.03
* Version:  V1.0
* ModifyRecord:
* ************************************************************************/  
void CMD_ImportRSAPriKey(void)
{
    UINT16 AppID = 0;
    UINT16 ConID = 0;
    UINT16 nByteLen = 0;
	...
	if(gApduHeader.P1 == TRUE)
	{
		...
		Node.FileType = FT_KEY;
		Node.FileSize = gApduBody.LC - 4;
		Node.FCP.FileID = SIGNPRIKEYID;
		strcpy(Node.FCP.Name, "SignRSAPriKey");
	...
	else if(gApduHeader.P1 == FALSE)
	{
		...
		Node.FileType = FT_KEY;
		Node.FileSize = gApduBody.LC - 4;
		Node.FCP.FileID = EXPRIKEYID;
		strcpy(Node.FCP.Name, "ExtRSAPriKey");
...
void CMD_ExtRSAPubKeyOPT()
{
	...
	tmpIndex += 4;
    memcpy(InData, gApduBody.DATA + tmpIndex, uDataLen);
    
    //使用公钥处理数据
    nRet = RSA_PubKeyOpt(gApduResp.DATA, &rLen, InData, uDataLen, &RsaPubKey);
    if (nRet != RT_OK)
    {                            
...
void CMD_ExtRSAPriKeyOPT()
{
    ...
    //私钥运算
    nRet = RSA_PriKeyOpt(gApduResp.DATA, &rLen, InData, uDataLen, &RsaPriKey);
    if (nRet != RT_OK)
    {                                       
...
void CMD_ExtRSAEnDecrypt()
{
	...
	memcpy(data, gApduBody.DATA, datalen);
	...
	memcpy((UINT8*)&RSAKey.bits, RSAKeydata, 2);
	index += 2;
    if (p11PriKey) RSAKey.modulus = NULL;
	else memcpy(RSAKey.modulus, RSAKeydata + index, nByteLen);
	index += nByteLen;   
	memcpy(RSAKey.publicExponent, RSAKeydata + index, 4);
	index += 4;
	memcpy(RSAKey.exponent, RSAKeydata + index, nByteLen);
...
void CMD_ImportSessionKey()
{
	...
	if (Node.ContainerType == RSA_CON)
	{
		...
		memcpy((UINT8*)&RSAKey.bits, RSAKeydata, 2);
		index += 2;
        if (1==p11PriKey) RSAKey.modulus = NULL;
		else memcpy(RSAKey.modulus, RSAKeydata+index, nByteLen);
		index += nByteLen;
		//memcpy(RSAKey.publicExponent, RSAKeydata+index, 4);
		index += 4;
...
/***************************************************************************
* Subroutine:	CMD_GenerateSymmKey()
* Function:		Generate a sessionkey and return key handle
* Author:   Syno common		 
* Date:		2015.11.22
* Version:  V1.0
* ModifyRecord:
* ************************************************************************/
void CMD_GenerateSymmKey(void)
{                  
    UINT16 keyId = 0;
	UINT32 AlgID = 0;
    UINT32 tempindex = 0;
    ...
    //保存会话密钥
	keyId = SaveSessionKey(tempbuf, keylen, AlgID);
	if (keyId == SAR_NO_ROOM)
	{                       
        DBG_ERR("The session cout meet the max!");
		SetCmdResult(SAR_NO_ROOM, 0);
		return;
	...
	gApduResp.DATA[0] = keyId & 0x00FF;
	gApduResp.DATA[1] = (keyId>>8) & 0x00FF;
...
UINT16 CMD_EncryptDecryptInit(CRYPTO_TYPE crypto_type)
{
	...
	{
		BOOL bAlgIsWrong = FALSE;
		switch (gSessionKey[keyid].algID)
		{
		case SGD_AES128:
			if (SGD_AES128_ECB == algId || SGD_AES128_CBC == algId)
			{
				gSessionKey[keyid].algID = algId;
			}
			break;
		case SGD_AES192:
			if (SGD_AES192_ECB == algId || SGD_AES192_CBC == algId)
			{
				gSessionKey[keyid].algID = algId;
			}
			break;
		case SGD_AES256:
			if (SGD_AES256_ECB == algId || SGD_AES256_CBC == algId)
			{
				gSessionKey[keyid].algID = algId;
			}
			break;
		case SGD_DES:
			if (SGD_DES_ECB == algId || SGD_DES_CBC == algId)
			{
				gSessionKey[keyid].algID = algId;
			}
			break;
		case SGD_DES3:
			if (SGD_DES3_ECB == algId || SGD_DES3_CBC == algId)
			{
				gSessionKey[keyid].algID = algId;
			}
			break;
    ...
    case SGD_DES_ECB:
		 DES_Init(gSessionKey[keyid].keyData, 8);
		 break;                                    
    case SGD_DES3_CBC:
		 memcpy(gSessionKey[keyid].IV, pBlockCipher.IV, pBlockCipher.IVLen);
    case SGD_DES3_ECB:
		 DES_Init(gSessionKey[keyid].keyData, 24);
		 break;
    case SGD_SM1_MAC:
	case SGD_SM1_CBC:
		 memcpy(gSessionKey[keyid].IV, pBlockCipher.IV, pBlockCipher.IVLen);
...
UINT16 CMD_EncryptDecryptUpdate(CRYPTO_TYPE flag, UINT16* pDataLen)
{
	...
	*pDataLen = gApduBody.LC - tempindex;
	...
	{        
	case SGD_DES_CBC: 
		DES_Run(flag!=CT_DEC ? DES_ENCRYPT:DES_DECRYPT, DES_CBC, data, data, *pDataLen, gSessionKey[keyid].IV);
		break;
	case SGD_DES_ECB:
		DES_Run(flag!=CT_DEC ? DES_ENCRYPT:DES_DECRYPT, DES_ECB, data, data, *pDataLen, NULL);
		break;  
	case SGD_DES3_CBC: 
		DES_Run(flag!=CT_DEC ? DES_ENCRYPT:DES_DECRYPT, DES3_CBC, data, data, *pDataLen, gSessionKey[keyid].IV);
		break;
	case SGD_DES3_ECB:
		DES_Run(flag!=CT_DEC ? DES_ENCRYPT:DES_DECRYPT, DES3_ECB, data, data, *pDataLen, NULL);
		break;
	case SGD_SM1_ECB:
	...
	case SGD_AES128_ECB:
		...
		break;
	case SGD_AES128_CBC:
		AES_Run(flag!=CT_DEC ? AES_ENCRYPT:AES_DECRYPT, AES128_CBC, data, data, *pDataLen, gSessionKey[keyid].IV);
		break;
	case SGD_AES192_ECB:
		_Aes_Run(flag!=CT_DEC ? AES_ENCRYPT:AES_DECRYPT, AES192_ECB, (UINT32)data, *pDataLen);
		break;
	case SGD_AES192_CBC:
		AES_Run(flag!=CT_DEC ? AES_ENCRYPT:AES_DECRYPT, AES192_CBC, data, data, *pDataLen, gSessionKey[keyid].IV);
		break;
	case SGD_AES256_ECB:
		_Aes_Run(flag!=CT_DEC ? AES_ENCRYPT:AES_DECRYPT, AES256_ECB, (UINT32)data, *pDataLen);
		break;
	case SGD_AES256_CBC:
		AES_Run(flag!=CT_DEC ? AES_ENCRYPT:AES_DECRYPT, AES256_CBC, data, data, *pDataLen, gSessionKey[keyid].IV);
		break;
	default:
		DBG_ERR("Session key alg ID is not supported! algID = 0x%x", gSessionKey[keyid].algID);
		nRet = SAR_UNSUPPORT_SESSION_KEY;
...
void CMD_DigestInit()
{
	...
	UINT16 hashIdx = 0;
	UINT32 keynBits = 0;
    UINT32 hash_tbl[4] = {SGD_SM3, SGD_SHA1, SGD_SHA256, SGD_MD5};
	UINT8 *ID = NULL;

	DBG_FUNC_BEGIN;

	hashIdx = gApduHeader.P2; // P2 must be value among {1:SM3,2:SHA1,3:SHA256,4:MD5}
    if ((hashIdx > 4) || (hashIdx <= 0))
    {
		DBG_ERR("P2 is wrong! gApduHeader.P2 = 0x%x", gApduHeader.P2);
        SetCmdResult(SAR_INDATAERR, 0);
        return;
...
void CMD_Digest()
{
    ...
    case SGD_SHA256:
        ...
        break;	 
    case SGD_MD5:   
		MD5_Update(&gMD5Context, data, datalen);
		MD5_Final(&gMD5Context, gApduResp.DATA + 4);
		datalen = 16;
        break;
	default:
        gHashAlgID = 0;

...
void  CZLibReadBuffer::init_file_header()
{
	...
	m_DiskFile->seek(m_FileBegPos,xcdsd_beg);
	...
	m_DiskFile->seek(old_pos,xcdsd_beg);
}
...
void  CZLibReadBuffer::cacheBlock(int index)
{
	...
	m_DiskFile->seek(readBlock.m_OffsetInFile, xcdsd_beg);
	readBlock.alloc();
...
void   CZLibReadBuffer::seekg(int _offset, xcd_seek_dir pos)
{
	if(pos == xcdsd_cur)
	{

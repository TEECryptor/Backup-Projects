...
/*
	...
	m_eCurStatus = eHttpTaskStatus_Ready;
	SendEventToApp(m_dwTaskID, eHTTPCBEvent_StatusChanged, eHttpTaskStatus_Ready, m_dwNetFileSize);
*/
...
//--------------------------------------------
void CHttpDownloadTask::SetThreadCnt(DWORD dwThreadCnt)
{
...
LONG CHttpDownloadTask::CreateDownloadThreads()
{
	...
	for (DWORD dwIndex = 0; dwIndex < m_stuDownloadInfo.dwThreadCnt; dwIndex++)
	{
		...
		m_pDownloadThreads[dwIndex].SetDownloadInfo(m_hConnect, m_hTargetFile, &info);
...
LONG CHttpDownloadTask::OnProgress()
{
	...
	// If speed has been 0 for 15 seconds, try to auto-restart 
	if (m_dwZeroBytesTime >= 15)
	{

...
public:
	...
	IComDocBase(){}
	virtual ~IComDocBase(){};
	virtual bool              open(IStreamIO* pReader,xcd_rwmode mode,int _offset = 0 , bool load_to_mem = false) = 0;
...
class  IComDocBaseCreator
{
public:
	virtual IComDocBase* createInstace()  = 0;
	virtual bool         support(const wchar_t* _ext) = 0;
};

class  xComDocBaseCreatorMgr
{
public:
	static       xComDocBaseCreatorMgr* singleton() ;
	bool         registCreator(IComDocBaseCreator* pCreator);
	bool         unregistCreator(IComDocBaseCreator* pCreator);
    IComDocBase* createInstance(const wchar_t* _ext);
protected:
	typedef std::vector<IComDocBaseCreator*>  vComDocBaseCreators;
	vComDocBaseCreators m_vComDocCreators;
};

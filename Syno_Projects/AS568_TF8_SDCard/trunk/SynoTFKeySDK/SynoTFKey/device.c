...
#ifndef _OPEN
#define _OPEN(pathname, flags)  (INT32 )CreateFileA(	\
	(LPCTSTR)pathname, \
...
INT32 SYNO_Open(CHAR *pcPathName)
{
    ...
    pHandle = SYNO_malloc(sizeof(FIL));
    if (NULL == pHandle)
    {
    ...
    if (SAR_OK != iRet)
    {
        SYNO_LogE("f_open err! iRet=<%d>", iRet);
        return -1;
    }
...
INT32 SYNO_Write(INT32 iHandle, void *pvBuf, INT32 iLen)
{
    ...
    if (SAR_OK != iRet)
    {
        SYNO_LogE("f_write err! iRet=<%d>", iRet);
        return -1;
    }
...
INT32 SYNO_Read(INT32 iHandle, void *pvBuf, INT32 iLen)
{
    ...
    if (SAR_OK != iRet)
    {
        SYNO_LogE("f_read err! iRet=<%d>", iRet);
        return -1;
    }
...
ULONG CreateIOBuf(UCHAR **ppIOBuf, ULONG ulBufLen)
{
    ...
    SYNO_LogD("system is WIN32 or SYNO_ASIDE!");
    *ppIOBuf = (UCHAR *)SYNO_malloc(ulBufLen);
#elif defined(LINUX)
    SYNO_LogD("system is Linux!");
...
ULONG OpenDevFile(CHAR *pcDevPath, VIRFILE_S *pstVirFile)
{
    ...
    SYNO_LogD("open acFilePath=<%s>", acFilePath);
    iFd = _OPEN(acFilePath, O_RDWR);
	if (-1 == iFd)
	{
        //SYNO_LogE("open command.sys errno=<%d>,<%s>", errno, strerror(errno));
        return SAR_FILEERR;
	}
	...
	if (-1 == iFd)
	{
        SYNO_LogE("open dataenc.sys err!");
	...
	if (-1 == iFd)
	{
        SYNO_LogE("open datadec.sys err!");
        CloseDevFile(&stVirFile);
        return SAR_FILEERR;
	}
...
ULONG OpenDefinedDev(HANDLE *pHandle)
{
    ...
    pstIODriver = (IODRIVER_S *)SYNO_calloc(1, sizeof(IODRIVER_S));
    if (NULL == pstIODriver)
    {
        LOG_ErrNo(SAR_MEMORYERR);
        return SAR_MEMORYERR;
...
void List_Files_Core(const char *path, int recursive)
{
    ...
    {
        SYNO_LogE("opendir error:%s", path);
    }
    closedir(pdir);
}
...
void DEV_CloseDevice(HANDLE pHandle)
{
    ...
    SYNO_free(pstIODriver->pucCmdBuf);
    SYNO_free(pstIODriver->pucDataBuf);
    SYNO_free(pstIODriver);
...
ULONG DEV_Write(RWDEVICE_S *pstRWDev, ULONG ulOffset, UCHAR *pucBuf, ULONG *pulDataLen)
{
    ...
    _LSEEK(iFd, ulOffset);
    SYNO_LogD("_WRITE ulBufLen = %d", *pulDataLen);
	iRetLen = _WRITE(iFd, pucWBuf, *pulDataLen);
	if (iRetLen <= 0)
	{
		*pulDataLen = 0;
        LOG_ErrNo(SAR_FILEERR);
        //SYNO_LogE("write device file err errno=<%d>,<%s>", errno, strerror(errno));
		return SAR_FILEERR;
	}
	*pulDataLen = iRetLen;
...
ULONG DEV_Read(RWDEVICE_S *pstRWDev, ULONG ulOffset, UCHAR *pucBuf, ULONG *pulDataLen)
{
    ...
    else
    {
		*pulDataLen = 0;
        LOG_ErrNo(SAR_FILEERR);
        //SYNO_LogE("read device file err errno=<%d>,<%s>", errno, strerror(errno));
		return SAR_FILEERR;
    }

 ...
 */
/*
 * RCSID:       $Id: error.h,v 1.6 2000/06/23 17:32:17 lbe Exp $
 * Source:      $Source: /usr/cvsroot/pkcs11/libgpkcs11/error.h,v $
 * Last Delta:  $Date: 2000/06/23 17:32:17 $ $Revision: 1.6 $ $Author: lbe $
...
CK_DECLARE_I_FUNCTION(CK_C_CHAR_PTR,CI_ErrorStr)(
  CK_RV rv
);
CK_DECLARE_I_FUNCTION(CK_C_CHAR_PTR,CI_MechanismStr)(
  CK_MECHANISM_TYPE rv
);
CK_DECLARE_I_FUNCTION(CK_C_CHAR_PTR,CI_AttributeStr)(
  CK_ATTRIBUTE_TYPE attrib
);
CK_DECLARE_I_FUNCTION(CK_ULONG,CI_AttributeNum)(
  CK_CHAR_PTR pAttribName
);
CK_DECLARE_I_FUNCTION(CK_CHAR_PTR, CI_PrintableByteStream)(
  CK_C_BYTE_PTR      stream,
  CK_ULONG         len
);

CK_DEFINE_I_FUNCTION(CK_CHAR_PTR, CI_ScanableByteStream)(
   CK_C_BYTE_PTR stream,
   CK_ULONG len
);

CK_DECLARE_I_FUNCTION(CK_CHAR_PTR, CI_ScanableMechanism)(
  CK_MECHANISM_PTR pMechanism
);

CK_DECLARE_I_FUNCTION(CK_CHAR_PTR, CI_PrintTemplate)(
  CK_ATTRIBUTE_PTR pTemplate,
  CK_ULONG         ulCount
);

#ifndef NO_LOGGING
CK_DECLARE_I_FUNCTION(void, CI_LogEntry)(
...
CK_DECLARE_I_FUNCTION(void, CI_SetLogingLevel)(
  CK_ULONG level
);
...
#else
#define CI_LogEntry(f,p,r,l) SYNO_LogI("%s %s",p, CI_ErrorStr(r))//do{}while(0)
#define CI_SetLogingLevel(l) do{}while(0)
#define CI_VarLogEntry(f,p,r,l,...) SYNO_LogI("%s %s",p, CI_ErrorStr(r))//do{}while(0)
#define CI_CodeFktEntry(f,p,...) SYNO_LogI("%s",p)//do{}while(0)
...
CK_DECLARE_I_FUNCTION(void,__TC_free)(
  void *ptr,
  unsigned int line,
  const char *file
);

CK_DECLARE_I_FUNCTION(void*,__TC_calloc)(
  size_t nelem,
  size_t elsize,
  unsigned int line,
  const char *file
...
CK_DECLARE_I_FUNCTION(void*,__TC_malloc)(
  size_t size,
  unsigned int line,
  const char *file
);
...
#ifndef WIN32
#define TC_free(handle) SYNO_free(handle)//free(handle)
#define TC_calloc(nelem, elsize) SYNO_calloc(nelem, elsize)//calloc(nelem,elsize)
#define TC_malloc(size) SYNO_malloc(size)//malloc(size)
#else

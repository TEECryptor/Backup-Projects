...
* Date         :	2012.07.11
* Version      :    v1.0
* Description  :    UART Hardware Abstract Layer compatible with SY03
* ModifyRecord :
****************************************************************************/
/***************************************************************************
 ...
 * Function:	UART Port initialization  
 * Input:       UART_MAP mPortMap-- Select which group of GPIO port as SPI
 *                                 UART_MAP_A4_A5--GPIOA[4]&GPIOA[5]
 *                                 UART_MAP_B2_B7--GPIOB[2]&GPIOB[7]
...
UINT8 UART_PortInit(UART_MAP mPortMap)
{
    switch(mPortMap)
	{
		case UART_MAP_A4_A5:
			// to select
            GpioRegs.GPAUSE0.bit.GPA4 = MUX_UART;	 //Select TxD pin for UART0	 GPA4
			GpioRegs.GPAUSE0.bit.GPA5 = MUX_UART;	 //Select RxD pin for UART0	 GPA5
			break;
		case UART_MAP_B2_B7:
			// to select
            GpioRegs.GPBUSE0.bit.GPB2 = MUX_UART;	  //Select TxD pin for UART0	 GPB2
			GpioRegs.GPBUSE0.bit.GPB7 = MUX_UART;	  //Select RxD pin for UART0	 GPB7
			break;
		default:
			return RT_FAIL;
	}
 ...
 * Input:		UINT8 bUsartNum -- 	USART number, only USART0 for SY03;

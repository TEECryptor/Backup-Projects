...
static CMIDI*	g_pMidiPlayer = NULL;

long VSTCALLBACK CMIDI::AudioMasterCallback(AEffect *effect, long opcode, long index, long value, void *ptr, float opt)
{
	if (!g_pMidiPlayer)
		return 0;
	else
		return g_pMidiPlayer->OnAudioMasterCallback(effect, opcode, index, value, ptr, opt);
}	
...
CMIDI::~CMIDI()
{
	...
	if (m_pMidiFileData)
	{
		delete []m_pMidiFileData;
		m_pMidiFileData = NULL;
	}
...
BOOL CMIDI::LoadMidi(LPCTSTR pszMidiFile, CWnd* pParent)
{
	OFSTRUCT os;
	memset(&os, 0, sizeof(OFSTRUCT));
	os.cBytes = sizeof(OFSTRUCT);
	HANDLE hMidiFile = CreateFile(	pszMidiFile, 
									GENERIC_READ, 
									FILE_SHARE_READ, 
									NULL,
									OPEN_EXISTING,
	...
	if (m_pMidiFileData)
		delete []m_pMidiFileData;
	m_pMidiFileData = new BYTE[dwFileSize];
	
	DWORD dwReadedBytes = 0;
	if (!ReadFile(hMidiFile, m_pMidiFileData, dwFileSize, &dwReadedBytes, NULL) ||
	...
	return Create(m_pMidiFileData, dwFileSize, pParent);
}

BOOL CMIDI::LoadEffect(LPCTSTR pszEffectFile)
{
	if (m_pEffect)
	{
		m_pEffect->EffClose();
	...
	if (!m_pEffect->Load(pszEffectFile))
	{
		delete m_pEffect;
		m_pEffect = NULL;
		return FALSE;
...
//	m_pEffect->bWantMidi = (pEffect->EffCanDo("receiveVstMidiEvent") == 1);
	m_pEffect->EffResume();                   /* then force resume.                */
	m_pEffect->EffSuspend();                  /* suspend again...                  */
	m_pEffect->EffSetBlockSize(4410);   /* and block size                    */
	m_pEffect->EffResume();                   /* then force resume.                */
...
void CMIDI :: OnMidiOutDone(MIDIHDR & rHdr) {
	...
	if( m_bPlaying && m_nEmptyBuffers ) {
		...
		CONVERTINFO* pInfo = &m_StreamBuffers[m_nCurrentBuffer];
		if (m_pEffect)
		{
			int iInNums = m_pEffect->GetInputNums();
			int iOutNums = m_pEffect->GetOutputNums();

			MIDIEVENT* lpMidiEvent = (MIDIEVENT *)( pInfo->mhBuffer.lpData
													+ pInfo->dwStartOffset
													+ pInfo->dwBytesRecorded );
			VstMidiEvent MidiEvent;
			memset(&MidiEvent, 0, sizeof(VstMidiEvent));
			MidiEvent.type = kVstMidiType;
			MidiEvent.byteSize = sizeof(VstMidiEvent);
			memcpy(MidiEvent.midiData, &lpMidiEvent->dwEvent, sizeof(DWORD));
			...
			m_pEffect->EffProcessEvents(&events);
		
			const int ciDataSize = 1024;
			float** ppIndata = NULL;
			float** ppOutdata = NULL;
			if (iInNums > 0)
			{
				ppIndata = new float*[iInNums];
				for (int i = 0; i < iInNums; i++)
				{
					ppIndata[i] = new float[ciDataSize];
					memset(ppIndata[i], 0, sizeof(float)*ciDataSize);
				}
			}
			if (iOutNums > 0)
			{
				ppOutdata = new float*[iOutNums];
				for (int i = 0; i < iOutNums; i++)
				{
					ppOutdata[i] = new float[ciDataSize];
					memset(ppOutdata[i], 0, sizeof(float)*ciDataSize);
				}
			}
			m_pEffect->EffProcessReplacing(ppIndata, ppOutdata, pInfo->mhBuffer.dwBytesRecorded);

/*
			BYTE* pData = new BYTE[pInfo->mhBuffer.dwBufferLength];
			memset(pData, 0, pInfo->mhBuffer.dwBufferLength);
			for (int iIndex = 0; iIndex < pInfo->mhBuffer.dwBytesRecorded; iIndex++)
				pData[iIndex] = pInfo->mhBuffer.lpData/256.;
			m_pEffect->EffProcessEvents();
						
			BOOL bReplacing = TRUE;
								
			if ((bReplacing) &&							
				(m_pEffect->pEffect->flags & effFlagsCanReplacing))
				m_pEffect->EffProcessReplacing(pData, pInfo->mhBuffer.lpData, pInfo->mhBuffer.dwBytesRecorded);
			else										
				m_pEffect->EffProcess(pData, pInfo->mhBuffer.lpData, pInfo->mhBuffer.dwBytesRecorded);			

			for (int iIndex = 0; iIndex < pInfo->mhBuffer.dwBytesRecorded; iIndex++)
				pInfo->mhBuffer.lpData[iIndex] *= 256;*/
...
long CMIDI :: OnAudioMasterCallback(AEffect *effect, long opcode, long index, long value, void *ptr, float opt)
{
	switch (opcode)
	{
	...
	case audioMasterWantMidi:
		return OnWantMidiEvents(value);
	case audioMasterNeedIdle:
		return OnNeedIdle();
	case audioMasterProcessEvents :
		MessageBox(NULL, _T("audioMasterProcessEvents"), _T("audioMasterProcessEvents"), MB_OK);
		break;
	}
...
long CMIDI::OnWantMidiEvents(long filter)
{
	if (m_pEffect)
	{

...
)
{
  ...
  rv = ICA_TOK_CreateObject(session_data, new_obj);
  if(rv != CKR_OK)
    {
      return rv;
    }
...
)
{
  ...
  ICA_LogD("object_list->entries=<%d>", session_data->object_list->entries);
  ...
  FUNC_BEGIN;
    ...
    for(i = 0 ; i <I_ATT_MAX_NUM ; i++)
    {
        ...
        if((temp_attr != NULL_PTR) && /* überhauptwas  und */
            ...
            overwrite))                                /* oder egal */
        {
            ICA_LogD("CI_ObjSetAttribute start...");
            ...
            ICA_LogD("CI_ObjSetAttribute ok!");
        }
    }

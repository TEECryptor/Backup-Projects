    ...
    ULONG *pulDevNameLen, ULONG *pulEvent)
{
	...
	/* 先枚举当前系统中已有的设备名列表 */
	SKF_EnumDev(TRUE, csLastDevNameList, &ulLastNameListLen);
	...
	{		
		...
		/* 获取新的设备列表 */
		ulRes = SKF_EnumDev(TRUE, csNewDevNameList, &ulNewNameListLen);
		if (SAR_OK != ulRes)
		{
			continue;
		}
    ...
    BYTE *pbData, ULONG ulDataLen, BYTE *pbEncryptedData, ULONG *pulEncryptedLen)
{
	...
	if (NULL == pbEncryptedData)
	{
		*pulEncryptedLen = MAX_DATA_BUF;
		return SAR_OK;
	}
	if (*pulEncryptedLen < MAX_DATA_BUF)
	{
        return SAR_INDATALENERR;
	}
	...
	if (g_ulPadLen > 0 && g_ulPadLen != 0x10)
	{
		lpPaddedData =  (BYTE*)SYNO_malloc(ulDataLen + g_ulPadLen);
		memcpy(lpPaddedData, pbData, ulDataLen);
		memcpy(lpPaddedData + ulDataLen, g_aucPadByte, g_ulPadLen);
		ulDataLen += g_ulPadLen;

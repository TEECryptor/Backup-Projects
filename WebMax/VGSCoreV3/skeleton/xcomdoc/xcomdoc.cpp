...
bool  xcomdoc::open(IStreamIO* pReader,xcd_rwmode mode,int _offset, bool load_to_mem,const wchar_t* _type)
{
	if(m_hFIle == NULL) m_hFIle = CDHFILE( CreateComDoc(_type) );
	IComDocBase* pDoc = CDCLASS(m_hFIle);
	assert(pDoc);
...
bool       xcomdoc::open(const wchar_t* file_name,xcd_rwmode mode,int _offset,bool load_to_mem ,bool not_create,const wchar_t* _type)
{
...
bool xcomdoc::open(xcomdocstream* pStream ,xcd_rwmode mode,int _offset ,bool load_to_mem,const wchar_t* _type)
{
	if(m_hFIle == NULL) m_hFIle = CDHFILE( CreateComDoc(_type) );
	IComDocBase* pDoc = CDCLASS(m_hFIle);
	assert(pDoc);
...
bool xcomdoc::open(const _xcd_int8* buf,  size_t buf_len,   xcd_rwmode mode,bool copyed_data,const wchar_t* _type)
{
	if(m_hFIle == NULL) 
	{
		m_hFIle = CDHFILE( CreateComDoc(_type) );
	}
	IComDocBase* pDoc = CDCLASS(m_hFIle);
	assert(pDoc);
...
bool       xcomdoc::create(const wchar_t* file_name,int hash_table_size,const wchar_t* _type)
{
	if(m_hFIle == NULL)
	{
		m_hFIle = CDHFILE( CreateComDoc(_type) );

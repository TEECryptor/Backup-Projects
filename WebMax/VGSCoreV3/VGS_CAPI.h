...
@return: TRUE,有相交.
*/
DLL_EXPORT bool GetWorldPointUnderLoc(DWORD sceneHandle,DWORD vpHandle,UINT ScreenX, UINT ScreenY,VECTOR3* planeNormal,VECTOR3* planePoint,VECTOR3* vec3WorldPoint);
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
/** 使用缺省参数创建一个相机
*/
DLL_EXPORT DWORD CreateCamera(DWORD sceneHandle, CHAR* strCameraName);
...
/** 获取所有二维元素的数目
*/
DLL_EXPORT UINT GetAllGUIObjectsCount();
...
// 获取所有Overlay的句柄列表
DLL_EXPORT void GetOverlayList(DWORD* vHandle);
...
//点选轴包围盒
DLL_EXPORT int GetModelAxisStatusUnderCursor(DWORD sceneHandle, DWORD modelHandle, DWORD hViewport, UINT nScreenX, UINT nScreenY);

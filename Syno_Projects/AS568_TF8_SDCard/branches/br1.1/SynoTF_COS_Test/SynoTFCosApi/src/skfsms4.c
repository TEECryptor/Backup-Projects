/***************************************************************************
 *	File name:skfsha1.c
 *	Introduce:The implmenet file for sms4 functions
 *	Author:Syno common
 *	Date:2015/07/18
 *  Version:v1.0
 **************************************************************************/

/*----------------------------------------------*
 * 包含头文件                                   *
 *----------------------------------------------*/
#include "skfsms4.h"

#include <stdio.h>
#include <stdlib.h>
#include <string.h>

UINT32 RK_ADDR[32];


const UINT8 SM4_S[256] = {
		};


UINT32 SM4_T2(UINT32 lIn)
{
   UINT8 val[4];
   UINT32 lVal,lOut;
   val[0]=lIn&0xff;
   val[1]=(lIn>>8)&0xff;
   val[2]=(lIn>>16)&0xff;
   val[3]=(lIn>>24)&0xff;
   lVal=SM4_S[val[0]]|(SM4_S[val[1]]<<8)|(SM4_S[val[2]]<<16)|(SM4_S[val[3]]<<24);
   lOut=lVal^((lVal<<13)|(lVal>>19))^((lVal<<23)|(lVal>>9));
   return lOut;
}


UINT32 SM4_T(UINT32 lIn)
{
   UINT8 val[4];
   UINT32 lVal,lOut;
   val[0]=lIn&0xff;
   val[1]=(lIn>>8)&0xff;
   val[2]=(lIn>>16)&0xff;
   val[3]=(lIn>>24)&0xff;
   lVal=SM4_S[val[0]]|(SM4_S[val[1]]<<8)|(SM4_S[val[2]]<<16)|(SM4_S[val[3]]<<24);
   lOut=lVal^((lVal<<2)|(lVal>>30))^((lVal<<10)|(lVal>>22))^((lVal<<18)|(lVal>>14))^((lVal<<24)|(lVal>>8));
   return lOut;
}

UINT32 SM4_F(UINT32 *X,UINT32 rk)
{
   UINT32 val;
   val=X[1]^X[2]^X[3]^rk;
   return (X[0]^SM4_T(val));
}


void SMS4ExternKey(UINT32 MK[4])
{
	UINT32 i;
	UINT32 *RK;
	UINT32 KK[4];
	UINT32 SS[42];

	const UINT32 FK[4]={0xA3B1BAC6,0x56AA3350,0x677D9197,0xB27022DC};
	UINT32 K[40];
	const UINT32 CK[32] = {
		0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
		0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
		0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
		0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
		0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
		0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
		0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
		0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
	};

	RK=RK_ADDR;
	memcpy(KK,MK,16);
	K[0]=MK[0]^FK[0];
	K[1]=MK[1]^FK[1];
	K[2]=MK[2]^FK[2];
	K[3]=MK[3]^FK[3];

    for(i=0;i<32;i++)
	{
		K[i+4]=K[i]^SM4_T2(K[i+1]^K[i+2]^K[i+3]^CK[i]);
		RK[i]=K[i+4];
		SS[i]=RK[i];
	}
	i=1;
}

void SMS4BlockEnc(UINT8 *pIn,UINT8 *pucOut)
{
	UINT32 i;
	UINT32 X[36];
	UINT32 *RK;

	RK=RK_ADDR;
	memcpy(X,pIn,16);
	//X[0]=pIn[0];X[1]=pIn[1];X[2]=pIn[2];X[3]=pIn[3];
	for(i=0;i<32;i++)
	{
	   	X[i+4]=SM4_F(X+i,RK[i]);
	}
	memcpy(pucOut,&X[35],4);
	memcpy(pucOut+4,&X[34],4);
	memcpy(pucOut+8,&X[33],4);
	memcpy(pucOut+12,&X[32],4);
	/*pOut[0]=X[35];
	pOut[1]=X[34];
	pOut[2]=X[33];
	pOut[3]=X[32];*/
}

void SMS4BlockDec(UINT8 *pIn,UINT8 *pucOut)
{
	UINT32 i;
	UINT32 X[36];
	UINT32 *RK;

	RK=RK_ADDR;
	memcpy(X,pIn,16);
//	X[0]=pIn[0];X[1]=pIn[1];X[2]=pIn[2];X[3]=pIn[3];
	for(i=0;i<32;i++)
	{
	   	X[i+4]=SM4_F(X+i,RK[31-i]);
	}
	memcpy(pucOut,&X[35],4);
	memcpy(pucOut+4,&X[34],4);
	memcpy(pucOut+8,&X[33],4);
	memcpy(pucOut+12,&X[32],4);
/*	pOut[0]=X[35];
	pOut[1]=X[34];
	pOut[2]=X[33];
	pOut[3]=X[32];*/
}

void Change(UINT8 *pBuf,UINT8 nLen)
{
	UINT8 i;
	UINT8 nTemp;
	for(i=0;i<nLen/4;i++)
	{
		nTemp=pBuf[i*4];
		pBuf[i*4]=pBuf[i*4+3];
		pBuf[i*4+3]=nTemp;
		nTemp=pBuf[i*4+1];
		pBuf[i*4+1]=pBuf[i*4+2];
		pBuf[i*4+2]=nTemp;
	}
}


UINT8 SMS4_Run_ECB(UINT8 nType, UINT8 *pIn, UINT8 *pucOut, UINT16 nDataLen)
{
    INT32 i;

	if(nDataLen%16) return RT_PARAM_ERR;

	if(nType==SMS4_ENCRYPT)
	{
	    for(i=0;i<nDataLen/16;i++)
	    {
			Change((pIn+i*16),16);
	    	SMS4BlockEnc((pIn+i*16),(pucOut+i*16));
	    	Change(pucOut+i*16,16);
	    }
	}
	else
	{
	    for(i=0;i<nDataLen/16;i++)
	    {
			Change((pIn+i*16),16);
	    	SMS4BlockDec((pIn+i*16),(pucOut+i*16));
	    	Change(pucOut+i*16,16);
	    }
	}

	return RT_OK;
}

 /***************************************************************************
* Subroutine:	SMS4_Run_CBC
* Function:		encrypt or decrypt by SMs4
* Input:		nType:	ENCRYPT or DECRYPT
                nMode	: SM1_CBC or  SM1_ECB
                pIn		: buffer will be encrypt or decrypt
                pOut		:result buffer
                nDataLen	:data size,must be multiple of 16
                pIV: init IV and the IV returned,must be 16 bytes
* Output:	success-RT_OK;
            fail-RT_FAIL;
            param error-RT_PARAM_ERROR;
* Description:
* Date:			2010.09.02
* ModifyRecord:
* *************************************************************************/
UINT8 SMS4_Run_CBC(UINT8 nType, UINT8 *pIn, UINT8 *pucOut, UINT16 nDataLen, UINT8 *pIV)
{
	UINT8 i,j;
	UINT8 TmpIV[16];

    if(nDataLen%16) return RT_PARAM_ERR;

	if(nType==SMS4_ENCRYPT)
	{

    	for (i = 0; i < (nDataLen / 16); i++)
    	{

    		for (j = 0; j < 16; j++)
    	   {
    		   *(pIn+i*16+j)= (*(pIn+i*16+j))^pIV[j];    // Complete the pre-result xor the plaintext.
           }
            Change((pIn+i*16),16);
    	    SMS4BlockEnc((pIn+i*16),(pucOut+i*16));
    	    Change(pucOut+i*16,16);
    		memcpy(pIV,pucOut+i*16,16);
    	}
	}
    else
    {
    	for (i = 0; i < (nDataLen / 16); i++)
    	{
            Change((pIn+i*16),16);
            memcpy(TmpIV,pIn+i*16,16);
    	    SMS4BlockDec((pIn+i*16),(pucOut+i*16));
    	    Change(pucOut+i*16,16);
    		for (j = 0; j < 16; j++)
    	   {
    		   *(pucOut+i*16+j)= (*(pucOut+i*16+j))^pIV[j];    // Complete the pre-result xor the plaintext.
           }

    		 memcpy(pIV,TmpIV,16);
    		 Change(pIV,16);
    	}

    }
	return RT_OK;
}

 /***************************************************************************
* Subroutine:	SMS4_Run_CFB
* Function:		encrypt or decrypt by SMS4
* Input:		nType:	ENCRYPT or DECRYPT
                nMode	: SM1_CBC or  SM1_ECB
                pIn		: buffer will be encrypt or decrypt
                pOut		:result buffer
                nDataLen	:data size,must be multiple of 16
                pIV: init IV and the IV returned,must be 16 bytes
* Output:	success-RT_OK;
            fail-RT_FAIL;
            param error-RT_PARAM_ERROR;
* Description:
* Date:			2010.09.02
* ModifyRecord:
* *************************************************************************/
UINT8 SMS4_Run_CFB(UINT8 nType, UINT8 *pIn, UINT8 *pucOut, UINT16 nDataLen, UINT8 *pIV)
{
	UINT8 i,j,k,n;

	UINT16 size;

    if(nDataLen%16) return RT_PARAM_ERR;

    size=16;

	if(nType==SMS4_ENCRYPT)
	{
		//In order to implement and/or function.And complete the SM1_CBC work.

	for (i = 0; i < nDataLen; i+=size)
	   {
        j=i/16, k=i%16;
        SMS4_Run_ECB(nType,pIV,pucOut+j*16,16);               //pIV变量加密
        for(n=k;n<(k+size);n++)
            {
        	   *(pucOut+j*16+n)=(*(pIn+j*16+n))^(*(pucOut+j*16+n));     // PIV的size个字节与明文异或加密，输出密文
            }
        memcpy(pIV,pIV+1,16-size);                                  // PIV 左移size个字节
        memcpy(pIV+(16-size),pucOut+j*16+k,size);                    //将新得的密文作为下一次加密常量PIV的高size个字节

        }
    }

    if(nType==SMS4_DECRYPT)
	{
		//In order to implement and/or function.And complete the SM1_CBC work.

     for (i = 0; i < nDataLen; i+=size)
	    {
        j=i/16, k=i%16;
        SMS4_Run_ECB(SMS4_ENCRYPT,pIV,pucOut+j*16,16);               //pIV变量加密
        for(n=k;n<(k+size);n++)
            {
        	   *(pucOut+j*16+n)=(*(pIn+j*16+n))^(*(pucOut+j*16+n));     // PIV的size个字节与明文异或加密，输出密文
            }
        memcpy(pIV,pIV+1,16-size);                                  // PIV 左移size个字节
        memcpy(pIV+(16-size),pIn+j*16+k,size);
        }                   //将新得的密文作为下一次加密常量PIV的高size个字节
    }
	return RT_OK;
}

 /***************************************************************************
* Subroutine:	SMS4_Run_OFB
* Function:		encrypt or decrypt by SM1
* Input:		nType:	ENCRYPT or DECRYPT
                nMode	: SM1_CBC or  SM1_ECB
                pIn		: buffer will be encrypt or decrypt
                pOut		:result buffer
                nDataLen	:data size,must be multiple of 16
                pIV: init IV and the IV returned,must be 16 bytes
* Output:	success-RT_OK;
            fail-RT_FAIL;
            param error-RT_PARAM_ERROR;
* Description:
* Date:			2010.09.02
* ModifyRecord:
* *************************************************************************/
UINT8 SMS4_Run_OFB(UINT8 nType, UINT8 *pIn, UINT8 *pucOut, UINT16 nDataLen, UINT8 *pIV)
{
	UINT8 i,j,k,n;
	UINT16 size;

	if(nDataLen%16) return RT_PARAM_ERR;

	size=16;

	if(nType==SMS4_ENCRYPT)
	{
		//In order to implement and/or function.And complete the SM1_CBC work.

	for (i = 0; i < nDataLen; i+=size)
	   {
        j=i/16, k=i%16;
        SMS4_Run_ECB(nType,pIV,pucOut+j*16,16);               //pIV变量加密
        memcpy(pIV,pIV+1,16-size);                                  // PIV 左移size个字节
        memcpy(pIV+(16-size),pucOut+j*16+k,size);                    //将新得的密文作为下一次加密常量PIV的高size个字节

        for(n=k;n<(k+size);n++)
            {
        	   *(pucOut+j*16+n)=(*(pIn+j*16+n))^(*(pucOut+j*16+n));     // PIV的size个字节与明文异或加密，输出密文
            }

        }
    }

    if(nType==SMS4_DECRYPT)
	{
		//In order to implement and/or function.And complete the SM1_CBC work.

     for (i = 0; i < nDataLen; i+=size)
	    {
        j=i/16, k=i%16;
        SMS4_Run_ECB(SMS4_ENCRYPT,pIV,pucOut+j*16,16);               //pIV变量加密
        memcpy(pIV,pIV+1,16-size);                                  // PIV 左移size个字节
        memcpy(pIV+(16-size),pucOut+j*16+k,size);                      //将新得的密文作为下一次加密常量PIV的高size个字节
        for(n=k;n<(k+size);n++)
            {
        	   *(pucOut+j*16+n)=(*(pIn+j*16+n))^(*(pucOut+j*16+n));     // PIV的size个字节与明文异或加密，输出密文
            }

        }
    }
	return RT_OK;
}

/***************************************************************************
* Subroutine:	SMS4_Init
* Function:		SMS4 init for key
* Input:		pEK,pAK		:Password ,all must be 16 bytes
* Output:	success-RT_OK;
            fail-RT_FAIL;
            param error-RT_PARAM_ERROR;
* Description:
* Date:			2010.09.02
* ModifyRecord:
* *************************************************************************/
UINT8 SMS4_Init(UINT8* pEK)
{
	UINT32 EK[4];
 	memcpy(EK,pEK,16);
	Change((UINT8*)EK,16);
	SMS4ExternKey((UINT32*)EK);
	return RT_OK;
}

/***************************************************************************
* Subroutine:	SMS4_Run
* Function:		encrypt or decrypt by SMS4
* Input:		nType:	ENCRYPT or DECRYPT
                nMode	: SM1_CBC or  SM1_ECB
                pIn		: buffer will be encrypt or decrypt
                pOut		:result buffer
                nDataLen	:data size,must be multiple of 16
                pIV: init IV and the IV returned,must be 16 bytes
* Output:	success-RT_OK;
            fail-RT_FAIL;
            param error-RT_PARAM_ERROR;
* Description:
* Date:			2010.09.02
* ModifyRecord:
* *************************************************************************/
UINT8 SMS4_Run(UINT8 nType , UINT8 nMode, UINT8 *pIn, UINT8 *pucOut, UINT16 nDataLen, UINT8 *pIV)
{

    switch(nMode)
   {
   	    case SMS4_ECB:
             return SMS4_Run_ECB(nType,pIn,pucOut,nDataLen);
             break;
        case SMS4_CBC:
             return SMS4_Run_CBC(nType,pIn,pucOut,nDataLen,pIV);
             break;
        case SMS4_OFB:
             return SMS4_Run_OFB(nType,pIn,pucOut,nDataLen,pIV);
             break;
        case SMS4_CFB:
             return SMS4_Run_CFB(nType,pIn,pucOut,nDataLen,pIV);
             break;
        default:
            return RT_PARAM_ERR;
   }

   return RT_OK;
}

UINT8 SMS4_ENC(UINT8 *pKey,UINT8 *pIn, UINT8 *pucOut, UINT16 nDatalen)
{

	if(nDatalen % 16)		//必须是16的倍数
		return -1;

	SMS4_Init(pKey);						//使用hash值作为密钥
	SMS4_Run(SMS4_ENCRYPT, SMS4_ECB, pIn, pucOut, nDatalen, NULL);
	return 0;
}

UINT8 SMS4_DEC(UINT8 *pKey,UINT8 *pIn, UINT8 *pucOut, UINT16 nDatalen)
{
	if(nDatalen % 16)		//必须是16的倍数
		return -1;

	SMS4_Init(pKey);		//使用hash值作为密钥

	SMS4_Run(SMS4_DECRYPT, SMS4_ECB, pIn, pucOut, nDatalen, NULL);
	return 0;
}


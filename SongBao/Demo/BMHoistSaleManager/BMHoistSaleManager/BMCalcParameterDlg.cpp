...
BEGIN_MESSAGE_MAP(CBMCalcParameterDlg, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_MG_LD, &CBMCalcParameterDlg::OnCbnSelchangeComboMgLd)
...
BOOL CBMCalcParameterDlg::OnInitDialog()
{
	...
	pComboBox->SetCurSel(0);
	//
	pComboBox = (CComboBox*)GetDlgItem(IDC_COMBO_MG_LD);
	iIndex = pComboBox->AddString(_T("0.14"));
	pComboBox->SetItemData(iIndex, 14);
	iIndex = pComboBox->AddString(_T("0.15"));
	pComboBox->SetItemData(iIndex, 15);
	iIndex = pComboBox->AddString(_T("0.16"));
	pComboBox->SetItemData(iIndex, 16);
	pComboBox->SetCurSel(0);
...
//-----------------------------------------------------------------------
void CBMCalcParameterDlg::OnCbnSelchangeComboMgLd()
{
	if (TRUE == m_bIsInitFinished)
	{
		SetParamHasChanged(IDC_COMBO_MG_LD);
	}
}
//-----------------------------------------------------------------------
...
void CBMCalcParameterDlg::InitParamDataStruct()
{
	...
	m_lstCalcParams.push_back(CalcParamData(_T("Calc_MG_ld"), eTableDataType_FLOAT, IDC_STATIC_MG_LD, IDC_COMBO_MG_LD));
}
//-----------------------------------------------------------------------

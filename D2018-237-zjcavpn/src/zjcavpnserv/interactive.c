...
InterfaceLuid(const char *iface_name, PNET_LUID luid)
{
    ...
    typedef NETIO_STATUS (*ConvertInterfaceAliasToLuidFn) (LPCWSTR, PNET_LUID);
...
DeleteAddress(PMIB_UNICASTIPADDRESS_ROW addr_row)
{
    ...
    typedef (*DeleteUnicastIpAddressEntryFn) (const PMIB_UNICASTIPADDRESS_ROW);
...
HandleAddressMessage(address_message_t *msg, undo_lists_t *lists)
{
    ...
    typedef (*CreateUnicastIpAddressEntryFn) (const PMIB_UNICASTIPADDRESS_ROW);
    typedef (*InitializeUnicastIpAddressEntryFn) (PMIB_UNICASTIPADDRESS_ROW);
...
DeleteRoute(PMIB_IPFORWARD_ROW2 fwd_row)
{
    ...
    typedef (*DeleteIpForwardEntry2Fn) (PMIB_IPFORWARD_ROW2);
...
HandleRouteMessage(route_message_t *msg, undo_lists_t *lists)
{
    ...
    typedef (*CreateIpForwardEntry2Fn) (PMIB_IPFORWARD_ROW2);
...
HandleFlushNeighborsMessage(flush_neighbors_message_t *msg)
{
    ...
    typedef (*FlushIpNetTable2Fn) (ADDRESS_FAMILY, NET_IFINDEX);
...
netsh_dns_cmd(const wchar_t *action, const wchar_t *proto, const wchar_t *if_name, const wchar_t *addr)
{
    ...
    /* Path of netsh */
    n = GetSystemDirectoryW(argv0, MAX_PATH);
    ...
    ncmdline = wcslen(fmt) + wcslen(if_name) + wcslen(addr) + 32 + 1;
...
HandleDNSConfigMessage(const dns_cfg_message_t *msg, undo_lists_t *lists)
{
    ...
    wide_name = utf8to16(msg->iface.name); /* utf8 to wide-char */
    if (!wide_name)
    {
        return ERROR_OUTOFMEMORY;
    }
...
HandleMessage(HANDLE pipe, DWORD bytes, DWORD count, LPHANDLE events, undo_lists_t *lists)
{
	...
	ack.header.message_id = -1;
	ack.error_number = ERROR_MESSAGE_DATA;
...
RunOpenvpn(LPVOID p)
{
	...
	ovpn_sa.nLength = sizeof(ovpn_sa),
    ovpn_sa.lpSecurityDescriptor = &ovpn_sd;
    ovpn_sa.bInheritHandle = FALSE;
    ...
    input_size = WideCharToMultiByte(CP_UTF8, 0, sud.std_input, -1, NULL, 0, NULL, NULL);
...
BOOL CmpHandle(LPVOID item, LPVOID hnd)
{
    return item == hnd;
}
...
ServiceStartInteractive(DWORD dwArgc, LPTSTR *lpszArgv)
{
    ...
    while (TRUE)
    {
        ...
        else
        {
            ...
            /* Worker thread ended */
            thread = RemoveListItem(&threads, CmpHandle, handles[error]);
            UpdateWaitHandles(&handles, &handle_count, io_event, exit_event, threads);
            CloseHandleEx(&thread);
        }
    }
